...
const char* const s_transformStreamInternalsTransformStreamDefaultControllerEnqueueCode = "(function (S,g){\"use strict\";const _=@getByIdDirectPrivate(S,\"stream\"),f=@getByIdDirectPrivate(_,\"readable\"),i=@getByIdDirectPrivate(f,\"readableStreamController\");if(@assert(i!==@undefined),!@readableStreamDefaultControllerCanCloseOrEnqueue(i))@throwTypeError(\"TransformStream.readable cannot close or enqueue\");try{@readableStreamDefaultControllerEnqueue(i,g)}catch(j){throw @transformStreamErrorWritableAndUnblockWrite(_,j),@getByIdDirectPrivate(f,\"storedError\")}const W=!@readableStreamDefaultControllerShouldCallPull(i);if(W!==@getByIdDirectPrivate(_,\"backpressure\"))@assert(W),@transformStreamSetBackpressure(_,!0)})\n";
...
const char* const s_transformStreamInternalsTransformStreamDefaultSinkCloseAlgorithmCode = "(function (_){\"use strict\";const I=@getByIdDirectPrivate(_,\"readable\"),f=@getByIdDirectPrivate(_,\"controller\"),j=@getByIdDirectPrivate(I,\"readableStreamController\"),k=@getByIdDirectPrivate(f,\"flushAlgorithm\");@assert(k!==@undefined);const u=@getByIdDirectPrivate(f,\"flushAlgorithm\").@call();@transformStreamDefaultControllerClearAlgorithms(f);const q=@newPromiseCapability(@Promise);return u.@then(()=>{if(@getByIdDirectPrivate(I,\"state\")===@streamErrored){q.@reject.@call(@undefined,@getByIdDirectPrivate(I,\"storedError\"));return}if(@readableStreamDefaultControllerCanCloseOrEnqueue(j))@readableStreamDefaultControllerClose(j);q.@resolve()},(v)=>{@transformStreamError(@getByIdDirectPrivate(f,\"stream\"),v),q.@reject.@call(@undefined,@getByIdDirectPrivate(I,\"storedError\"))}),q.@promise})\n";
...
const char* const s_processObjectInternalsGetStdinStreamCode = "(function (Y,H,I){\"use strict\";var J={path:\"node:process\",require:H},j=(z)=>J.require(z),{Duplex:U,eos:K,destroy:L}=j(\"node:stream\"),M=class z extends U{#Y;#$;#j;#z=!0;#G=!1;#H=!0;#I;#J;#K;get isTTY(){return j(\"tty\").isatty(Y)}get fd(){return Y}constructor(){super({readable:!0,writable:!0})}#L(G){const V=this.#J;if(this.#J=null,V)V(G);else if(G)this.destroy(G);else if(!this.#z&&!this.#H)this.destroy()}_destroy(G,V){if(!G&&this.#J!==null){var N=class P extends Error{constructor(Q=\"The operation was aborted\",T=void 0){if(T!==void 0&&typeof T!==\"object\")throw new Error(`Invalid AbortError options:\\n\\n${JSON.stringify(T,null,2)}`);super(Q,T);this.code=\"ABORT_ERR\",this.name=\"AbortError\"}};G=new N}if(this.#J===null)V(G);else if(this.#J=V,this.#j)L(this.#j,G)}setRawMode(G){}on(G,V){if(G===\"readable\")this.ref(),this.#G=!0;return super.on(G,V)}pause(){return this.unref(),super.pause()}resume(){return this.ref(),super.resume()}ref(){this.#Y\?\?=I.stdin.stream().getReader(),this.#$\?\?=setInterval(()=>{},1<<30)}unref(){if(this.#$)clearInterval(this.#$),this.#$=null}async#M(){try{var G,V;const N=this.#Y.readMany();if(!N\?.then)({done:G,value:V}=N);else({done:G,value:V}=await N);if(!G){this.push(V[0]);const P=V.length;for(let Q=1;Q<P;Q++)this.push(V[Q])}else this.push(null),this.pause(),this.#z=!1,this.#L()}catch(N){this.#z=!1,this.#L(N)}}_read(G){if(this.#G)this.unref(),this.#G=!1;this.#M()}#N(){var{createWriteStream:G}=j(\"node:fs\"),V=this.#j=G(\"/dev/fd/0\");return V.on(\"finish\",()=>{if(this.#I){const N=this.#I;this.#I=null,N()}}),V.on(\"drain\",()=>{if(this.#K){const N=this.#K;this.#K=null,N()}}),K(V,(N)=>{if(this.#H=!1,N)L(V,N);this.#L(N)}),V}_write(G,V,N){var P=this.#j;if(!P)P=this.#N();if(P.write(G,V))N();else this.#K=N}_final(G){this.#j.end(),this.#I=(...V)=>G(...V)}};return new M})\n";
...
const char* const s_readableStreamInternalsPipeToShutdownCode = "(function (d){\"use strict\";if(d.shuttingDown)return;d.shuttingDown=!0;const m=arguments.length>1,u=arguments[1],I=()=>{if(m)@pipeToFinalize(d,u);else @pipeToFinalize(d)};if(@getByIdDirectPrivate(d.destination,\"state\")===\"writable\"&&!@writableStreamCloseQueuedOrInFlight(d.destination)){d.pendingReadPromiseCapability.@promise.@then(()=>{d.pendingWritePromise.@then(I,I)},(_)=>@pipeToFinalize(d,_));return}I()})\n";
...
const char* const s_readableStreamInternalsReadDirectStreamCode = "(function (C,_,f){\"use strict\";@putByIdDirectPrivate(C,\"underlyingSource\",@undefined),@putByIdDirectPrivate(C,\"start\",@undefined);function B(z,x){var v=f\?.cancel;if(v){try{var A=v.@apply(f,x);if(@isPromise(A))@markPromiseAsHandled(A)}catch(j){}f=@undefined}if(z)if(@putByIdDirectPrivate(z,\"readableStreamController\",@undefined),@putByIdDirectPrivate(z,\"reader\",@undefined),x)@putByIdDirectPrivate(z,\"state\",@streamErrored),@putByIdDirectPrivate(z,\"storedError\",x);else @putByIdDirectPrivate(z,\"state\",@streamClosed)}if(!f.pull){B(C);return}if(!@isCallable(f.pull))B(C),@throwTypeError(\"pull is not a function\");@putByIdDirectPrivate(C,\"readableStreamController\",_);const w=@getByIdDirectPrivate(C,\"highWaterMark\");_.start({highWaterMark:!w||w<64\?64:w}),@startDirectStream.@call(_,C,f.pull,B),@putByIdDirectPrivate(C,\"reader\",{});var q=f.pull(_);if(_=@undefined,q&&@isPromise(q))return q.@then(()=>{})})\n";
...
const char* const s_readableStreamInternalsOnCloseDirectStreamCode = "(function (c){\"use strict\";var v=this.@controlledReadableStream;if(!v||@getByIdDirectPrivate(v,\"state\")!==@streamReadable)return;if(this._deferClose!==0){this._deferClose=1,this._deferCloseReason=c;return}if(@putByIdDirectPrivate(v,\"state\",@streamClosing),typeof this.@underlyingSource.close===\"function\")try{this.@underlyingSource.close.@call(this.@underlyingSource,c)}catch(S){}var y;try{y=this.@sink.end(),@putByIdDirectPrivate(this,\"sink\",@undefined)}catch(S){if(this._pendingRead){var B=this._pendingRead;this._pendingRead=@undefined,@rejectPromise(B,S)}@readableStreamError(v,S);return}this.error=this.flush=this.write=this.close=this.end=@onReadableStreamDirectControllerClosed;var C=@getByIdDirectPrivate(v,\"reader\");if(C&&@isReadableStreamDefaultReader(C)){var j=this._pendingRead;if(j&&@isPromise(j)&&y\?.byteLength){this._pendingRead=@undefined,@fulfillPromise(j,{value:y,done:!1}),@readableStreamClose(v);return}}if(y\?.byteLength){var b=@getByIdDirectPrivate(C,\"readRequests\");if(b\?.isNotEmpty()){@readableStreamFulfillReadRequest(v,y,!1),@readableStreamClose(v);return}@putByIdDirectPrivate(v,\"state\",@streamReadable),this.@pull=()=>{var S=@createFulfilledPromise({value:y,done:!1});return y=@undefined,@readableStreamClose(v),v=@undefined,S}}else if(this._pendingRead){var B=this._pendingRead;this._pendingRead=@undefined,@putByIdDirectPrivate(this,\"pull\",@noopDoneFunction),@fulfillPromise(B,{value:@undefined,done:!0})}@readableStreamClose(v)})\n";
...
const char* const s_readableStreamInternalsCreateTextStreamCode = "(function (q){\"use strict\";var x,z=[],A=!1,v=!1,F=\"\",j=@toLength(0),G=@newPromiseCapability(@Promise),w=!1;return x={start(){},write(C){if(typeof C===\"string\"){var E=@toLength(C.length);if(E>0)F+=C,A=!0,j+=E;return E}if(!C||!(@ArrayBuffer.@isView(C)||C instanceof @ArrayBuffer))@throwTypeError(\"Expected text, ArrayBuffer or ArrayBufferView\");const _=@toLength(C.byteLength);if(_>0)if(v=!0,F.length>0)@arrayPush(z,F,C),F=\"\";else @arrayPush(z,C);return j+=_,_},flush(){return 0},end(){if(w)return\"\";return x.fulfill()},fulfill(){w=!0;const C=x.finishInternal();return @fulfillPromise(G.@promise,C),C},finishInternal(){if(!A&&!v)return\"\";if(A&&!v)return F;if(v&&!A)return new globalThis.TextDecoder().decode(@Bun.concatArrayBuffers(z));var C=new @Bun.ArrayBufferSink;C.start({highWaterMark:j,asUint8Array:!0});for(let E of z)C.write(E);if(z.length=0,F.length>0)C.write(F),F=\"\";return new globalThis.TextDecoder().decode(C.end())},close(){try{if(!w)w=!0,x.fulfill()}catch(C){}}},[x,G]})\n";
...
const char* const s_readableStreamInternalsReadableStreamErrorCode = "(function (_,i){\"use strict\";@assert(@isReadableStream(_)),@assert(@getByIdDirectPrivate(_,\"state\")===@streamReadable),@putByIdDirectPrivate(_,\"state\",@streamErrored),@putByIdDirectPrivate(_,\"storedError\",i);const f=@getByIdDirectPrivate(_,\"reader\");if(!f)return;if(@isReadableStreamDefaultReader(f)){const v=@getByIdDirectPrivate(f,\"readRequests\");@putByIdDirectPrivate(f,\"readRequests\",@createFIFO());for(var h=v.shift();h;h=v.shift())@rejectPromise(h,i)}else{@assert(@isReadableStreamBYOBReader(f));const v=@getByIdDirectPrivate(f,\"readIntoRequests\");@putByIdDirectPrivate(f,\"readIntoRequests\",@createFIFO());for(var h=v.shift();h;h=v.shift())@rejectPromise(h,i)}@getByIdDirectPrivate(f,\"closedPromiseCapability\").@reject.@call(@undefined,i);const c=@getByIdDirectPrivate(f,\"closedPromiseCapability\").@promise;@markPromiseAsHandled(c)})\n";
...
const char* const s_readableStreamInternalsReadableStreamCancelCode = "(function (i,_){\"use strict\";@putByIdDirectPrivate(i,\"disturbed\",!0);const d=@getByIdDirectPrivate(i,\"state\");if(d===@streamClosed)return @Promise.@resolve();if(d===@streamErrored)return @Promise.@reject(@getByIdDirectPrivate(i,\"storedError\"));@readableStreamClose(i);var h=@getByIdDirectPrivate(i,\"readableStreamController\"),j=h.@cancel;if(j)return j(h,_).@then(function(){});var p=h.close;if(p)return @Promise.@resolve(h.close(_));return @Promise.@reject(@makeTypeError(\"ReadableStreamController has no cancel or close method\"))})\n";
...
const char* const s_readableStreamInternalsLazyLoadStreamCode = "(function (f,b){\"use strict\";var E=@getByIdDirectPrivate(f,\"bunNativeType\"),F=@getByIdDirectPrivate(f,\"bunNativePtr\"),B=@lazyStreamPrototypeMap.@get(E);if(B===@undefined){let Y=function(A){var{c:x,v:Q}=this;this.c=@undefined,this.v=@undefined,K(A,x,Q)},Z=function(A){try{A.close()}catch(x){globalThis.reportError(x)}},_=function(A,x,Q,W){W[0]=!1;var U;try{U=j(A,Q,W)}catch(y){return x.error(y)}return K(U,x,Q)};var P=Y,X=Z,O=_,[j,G,z,H,D,m,I]=@lazyLoad(E),J=[!1],K;K=function A(x,Q,W){if(x&&@isPromise(x))return x.then(Y.bind({c:Q,v:W}),(U)=>Q.error(U));else if(typeof x===\"number\")if(W&&W.byteLength===x&&W.buffer===Q.byobRequest\?.view\?.buffer)Q.byobRequest.respondWithNewView(W);else Q.byobRequest.respond(x);else if(x.constructor===@Uint8Array)Q.enqueue(x);if(J[0]||x===!1)@enqueueJob(Z,Q),J[0]=!1};const p=D\?new FinalizationRegistry(D):null;B=class A{constructor(x,Q,W){if(this.#f=x,this.#b={},this.pull=this.#j.bind(this),this.cancel=this.#m.bind(this),this.autoAllocateChunkSize=Q,W!==@undefined)this.start=(U)=>{U.enqueue(W)};if(p)p.register(this,x,this.#b)}#b;pull;cancel;start;#f;type=\"bytes\";autoAllocateChunkSize=0;static startSync=G;#j(x){var Q=this.#f;if(!Q){x.close();return}_(Q,x,x.byobRequest.view,J)}#m(x){var Q=this.#f;p&&p.unregister(this.#b),m&&m(Q,!1),z(Q,x)}static deinit=D;static drain=I},@lazyStreamPrototypeMap.@set(E,B)}const L=B.startSync(F,b);var q;const{drain:M,deinit:N}=B;if(M)q=M(F);if(L===0){if(D&&F&&@enqueueJob(D,F),(q\?.byteLength\?\?0)>0)return{start(Y){Y.enqueue(q),Y.close()},type:\"bytes\"};return{start(Y){Y.close()},type:\"bytes\"}}return new B(F,L,q)})\n";
...
const char* const s_readableStreamInternalsReadableStreamToArrayBufferDirectCode = "(function (_,q){\"use strict\";var v=new @Bun.ArrayBufferSink;@putByIdDirectPrivate(_,\"underlyingSource\",@undefined);var w=@getByIdDirectPrivate(_,\"highWaterMark\");v.start(w\?{highWaterMark:w}:{});var x=@newPromiseCapability(@Promise),z=!1,A=q.pull,B=q.close,C={start(){},close(O){if(!z){if(z=!0,B)B();@fulfillPromise(x.@promise,v.end())}},end(){if(!z){if(z=!0,B)B();@fulfillPromise(x.@promise,v.end())}},flush(){return 0},write:v.write.bind(v)},j=!1;try{const O=A(C);if(O&&@isObject(O)&&@isPromise(O))return async function(D,F,G){while(!z)await G(D);return await F}(C,promise,A);return x.@promise}catch(O){return j=!0,@readableStreamError(_,O),@Promise.@reject(O)}finally{if(!j&&_)@readableStreamClose(_);C=B=v=A=_=@undefined}})\n";
...
const char* const s_readableStreamInternalsReadableStreamDefineLazyIteratorsCode = "(function (h){\"use strict\";var z=globalThis.Symbol.asyncIterator,k=async function*x(B,D){var F=B.getReader(),i;try{while(!0){var j,G;const q=F.readMany();if(@isPromise(q))({done:j,value:G}=await q);else({done:j,value:G}=q);if(j)return;yield*G}}catch(q){i=q}finally{if(F.releaseLock(),!D)B.cancel(i);if(i)throw i}},g=function x(){return k(this,!1)},w=function x({preventCancel:B=!1}={preventCancel:!1}){return k(this,B)};return @Object.@defineProperty(h,z,{value:g}),@Object.@defineProperty(h,\"values\",{value:w}),h})\n";
...
const char* const s_eventStreamGetEventStreamCode = "(function (){\"use strict\";return class h extends @ReadableStream{#h;constructor(m){super({type:\"direct\",pull:(_)=>{this.#h=_,m\?.start\?.(this)},cancel:()=>{m\?.cancel\?.(this),this.#h=@undefined}})}send(m,_){var i=this.#h;if(!i)throw new Error(\"EventStream has ended\");if(!_)_=m,m=@undefined;else if(m===\"message\")m=@undefined;if(_===@undefined)@throwTypeError(\"EventStream.send() requires a data argument\");if(typeof _===\"string\")i.write(\"data: \"+_.replace(/\\n/g,\"\\ndata: \")+\"\\n\\n\");else{if(m)i.write(\"event: \"+m+\"\\n\");i.write(\"data: \"+JSON.stringify(_)+\"\\n\\n\")}i.flush()}}})\n";
...
const char* const s_readableStreamDefaultReaderReadManyCode = "(function (){\"use strict\";if(!@isReadableStreamDefaultReader(this))@throwTypeError(\"ReadableStreamDefaultReader.readMany() should not be called directly\");const S=@getByIdDirectPrivate(this,\"ownerReadableStream\");if(!S)@throwTypeError(\"readMany() called on a reader owned by no readable stream\");const C=@getByIdDirectPrivate(S,\"state\");if(@putByIdDirectPrivate(S,\"disturbed\",!0),C===@streamClosed)return{value:[],size:0,done:!0};else if(C===@streamErrored)throw @getByIdDirectPrivate(S,\"storedError\");var J=@getByIdDirectPrivate(S,\"readableStreamController\"),E=@getByIdDirectPrivate(J,\"queue\");if(!E)return J.@pull(J).@then(function({done:N,value:d}){return N\?{done:!0,value:[],size:0}:{value:[d],size:1,done:!1}});const F=E.content;var D=E.size,T=F.toArray(!1),j=T.length;if(j>0){var k=@newArrayWithSize(j);if(@isReadableByteStreamController(J)){{const N=T[0];if(!(@ArrayBuffer.@isView(N)||N instanceof @ArrayBuffer))@putByValDirect(k,0,new @Uint8Array(N.buffer,N.byteOffset,N.byteLength));else @putByValDirect(k,0,N)}for(var K=1;K<j;K++){const N=T[K];if(!(@ArrayBuffer.@isView(N)||N instanceof @ArrayBuffer))@putByValDirect(k,K,new @Uint8Array(N.buffer,N.byteOffset,N.byteLength));else @putByValDirect(k,K,N)}}else{@putByValDirect(k,0,T[0].value);for(var K=1;K<j;K++)@putByValDirect(k,K,T[K].value)}if(@resetQueue(@getByIdDirectPrivate(J,\"queue\")),@getByIdDirectPrivate(J,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(J,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(J))@readableStreamDefaultControllerCallPullIfNeeded(J);else if(@isReadableByteStreamController(J))@readableByteStreamControllerCallPullIfNeeded(J);return{value:k,size:D,done:!1}}var U=(N)=>{if(N.done)return{value:[],size:0,done:!0};var d=@getByIdDirectPrivate(S,\"readableStreamController\"),_=@getByIdDirectPrivate(d,\"queue\"),G=[N.value].concat(_.content.toArray(!1)),x=G.length;if(@isReadableByteStreamController(d))for(var H=0;H<x;H++){const B=G[H];if(!(@ArrayBuffer.@isView(B)||B instanceof @ArrayBuffer)){const{buffer:Q,byteOffset:w,byteLength:I}=B;@putByValDirect(G,H,new @Uint8Array(Q,w,I))}}else for(var H=1;H<x;H++)@putByValDirect(G,H,G[H].value);var A=_.size;if(@resetQueue(_),@getByIdDirectPrivate(d,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(d,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(d))@readableStreamDefaultControllerCallPullIfNeeded(d);else if(@isReadableByteStreamController(d))@readableByteStreamControllerCallPullIfNeeded(d);return{value:G,size:A,done:!1}},W=J.@pull(J);if(W&&@isPromise(W))return W.@then(U);return U(W)})\n";
...
const char* const s_streamInternalsCreateFIFOCode = "(function (){\"use strict\";var x=@Array.prototype.slice;class g{constructor(){this._head=0,this._tail=0,this._capacityMask=3,this._list=@newArrayWithSize(4)}_head;_tail;_capacityMask;_list;size(){if(this._head===this._tail)return 0;if(this._head<this._tail)return this._tail-this._head;else return this._capacityMask+1-(this._head-this._tail)}isEmpty(){return this.size()==0}isNotEmpty(){return this.size()>0}shift(){var{_head:k,_tail:w,_list:z,_capacityMask:A}=this;if(k===w)return @undefined;var B=z[k];if(@putByValDirect(z,k,@undefined),k=this._head=k+1&A,k<2&&w>1e4&&w<=z.length>>>2)this._shrinkArray();return B}peek(){if(this._head===this._tail)return @undefined;return this._list[this._head]}push(k){var w=this._tail;if(@putByValDirect(this._list,w,k),this._tail=w+1&this._capacityMask,this._tail===this._head)this._growArray()}toArray(k){var w=this._list,z=@toLength(w.length);if(k||this._head>this._tail){var A=@toLength(this._head),B=@toLength(this._tail),E=@toLength(z-A+B),F=@newArrayWithSize(E),G=0;for(var b=A;b<z;b++)@putByValDirect(F,G++,w[b]);for(var b=0;b<B;b++)@putByValDirect(F,G++,w[b]);return F}else return x.@call(w,this._head,this._tail)}clear(){this._head=0,this._tail=0,this._list.fill(@undefined)}_growArray(){if(this._head)this._list=this.toArray(!0),this._head=0;this._tail=@toLength(this._list.length),this._list.length<<=1,this._capacityMask=this._capacityMask<<1|1}shrinkArray(){this._list.length>>>=1,this._capacityMask>>>=1}}return new g})\n";
...
const char* const s_importMetaObjectCreateRequireCacheCode = "(function (){\"use strict\";var _=new Map,t={};return new Proxy(t,{get(u,h){const g=@requireMap.@get(h);if(g)return g;const c=@Loader.registry.@get(h);if(c\?.evaluated){const f=@Loader.getModuleNamespaceObject(c.module),b=f[@commonJSSymbol]===0||f.default\?.[@commonJSSymbol]\?f.default:f,L=@createCommonJSModule(h,b,!0);return @requireMap.@set(h,L),L}return t[h]},set(u,h,g){return @requireMap.@set(h,g),!0},has(u,h){return @requireMap.@has(h)||@Loader.registry.@has(h)},deleteProperty(u,h){return _.@delete(h),@requireMap.@delete(h),@Loader.registry.@delete(h),!0},ownKeys(u){var h=[...@requireMap.@keys()];const g=[...@Loader.registry.@keys()];for(let c of g)if(!h.includes(c))@arrayPush(h,c);return h},getPrototypeOf(u){return null},getOwnPropertyDescriptor(u,h){if(@requireMap.@has(h)||@Loader.registry.@has(h))return{configurable:!0,enumerable:!0}}})})\n";
...
const char* const s_readableStreamConsumeReadableStreamCode = "(function (F,B,A){\"use strict\";const D=globalThis.Symbol.for(\"Bun.consumeReadableStreamPrototype\");var j=globalThis[D];if(!j)j=globalThis[D]=[];var G=j[B];if(G===@undefined){var[H,I,k,J,K,q]=globalThis[globalThis.Symbol.for(\"Bun.lazy\")](B);G=class L{handleError;handleClosed;processResult;constructor(_,N){this.#_=N,this.#F=_,this.#$=!1,this.handleError=this._handleError.bind(this),this.handleClosed=this._handleClosed.bind(this),this.processResult=this._processResult.bind(this),_.closed.then(this.handleClosed,this.handleError)}_handleClosed(){if(this.#$)return;this.#$=!0;var _=this.#_;this.#_=0,J(_),q(_)}_handleError(_){if(this.#$)return;this.#$=!0;var N=this.#_;this.#_=0,I(N,_),q(N)}#_;#$=!1;#F;_handleReadMany({value:_,done:N,size:x}){if(N){this.handleClosed();return}if(this.#$)return;k(this.#_,_,N,x)}read(){if(!this.#_)return @throwTypeError(\"ReadableStreamSink is already closed\");return this.processResult(this.#F.read())}_processResult(_){if(_&&@isPromise(_)){if(@getPromiseInternalField(_,@promiseFieldFlags)&@promiseStateFulfilled){const x=@getPromiseInternalField(_,@promiseFieldReactionsOrResult);if(x)_=x}}if(_&&@isPromise(_))return _.then(this.processResult,this.handleError),null;if(_.done)return this.handleClosed(),0;else if(_.value)return _.value;else return-1}readMany(){if(!this.#_)return @throwTypeError(\"ReadableStreamSink is already closed\");return this.processResult(this.#F.readMany())}};const w=B+1;if(j.length<w)j.length=w;@putByValDirect(j,B,G)}if(@isReadableStreamLocked(A))@throwTypeError(\"Cannot start reading from a locked stream\");return new G(A.getReader(),F)})\n";
...
const char* const s_readableStreamPipeThroughCode = "(function (_,O){\"use strict\";const u=_,c=u[\"readable\"];if(!@isReadableStream(c))@throwTypeError(\"readable should be ReadableStream\");const j=u[\"writable\"],d=@getInternalWritableStream(j);if(!@isWritableStream(d))@throwTypeError(\"writable should be WritableStream\");let k=!1,S=!1,y=!1,h;if(!@isUndefinedOrNull(O)){if(!@isObject(O))@throwTypeError(\"options must be an object\");if(S=!!O[\"preventAbort\"],y=!!O[\"preventCancel\"],k=!!O[\"preventClose\"],h=O[\"signal\"],h!==@undefined&&!@isAbortSignal(h))@throwTypeError(\"options.signal must be AbortSignal\")}if(!@isReadableStream(this))throw @makeThisTypeError(\"ReadableStream\",\"pipeThrough\");if(@isReadableStreamLocked(this))@throwTypeError(\"ReadableStream is locked\");if(@isWritableStreamLocked(d))@throwTypeError(\"WritableStream is locked\");return @readableStreamPipeToWritableStream(this,d,k,S,y,h),c})\n";
...
const char* const s_readableStreamPipeToCode = "(function (m){\"use strict\";if(!@isReadableStream(this))return @Promise.@reject(@makeThisTypeError(\"ReadableStream\",\"pipeTo\"));if(@isReadableStreamLocked(this))return @Promise.@reject(@makeTypeError(\"ReadableStream is locked\"));let _=@argument(1),j=!1,E=!1,b=!1,R;if(!@isUndefinedOrNull(_)){if(!@isObject(_))return @Promise.@reject(@makeTypeError(\"options must be an object\"));try{E=!!_[\"preventAbort\"],b=!!_[\"preventCancel\"],j=!!_[\"preventClose\"],R=_[\"signal\"]}catch(S){return @Promise.@reject(S)}if(R!==@undefined&&!@isAbortSignal(R))return @Promise.@reject(@makeTypeError(\"options.signal must be AbortSignal\"))}const u=@getInternalWritableStream(m);if(!@isWritableStream(u))return @Promise.@reject(@makeTypeError(\"ReadableStream pipeTo requires a WritableStream\"));if(@isWritableStreamLocked(u))return @Promise.@reject(@makeTypeError(\"WritableStream is locked\"));return @readableStreamPipeToWritableStream(this,u,j,E,b,R)})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerCancelCode = "(function (p,_){\"use strict\";var a=@getByIdDirectPrivate(p,\"pendingPullIntos\"),u=a.peek();if(u)u.bytesFilled=0;return @putByIdDirectPrivate(p,\"queue\",@newQueue()),@promiseInvokeOrNoop(@getByIdDirectPrivate(p,\"underlyingByteSource\"),\"cancel\",[_])})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerPullCode = "(function (_){\"use strict\";const h=@getByIdDirectPrivate(_,\"controlledReadableStream\");if(@assert(@readableStreamHasDefaultReader(h)),@getByIdDirectPrivate(_,\"queue\").content\?.isNotEmpty()){const d=@getByIdDirectPrivate(_,\"queue\").content.shift();@getByIdDirectPrivate(_,\"queue\").size-=d.byteLength,@readableByteStreamControllerHandleQueueDrain(_);let P;try{P=new @Uint8Array(d.buffer,d.byteOffset,d.byteLength)}catch(R){return @Promise.@reject(R)}return @createFulfilledPromise({value:P,done:!1})}if(@getByIdDirectPrivate(_,\"autoAllocateChunkSize\")!==@undefined){let d;try{d=@createUninitializedArrayBuffer(@getByIdDirectPrivate(_,\"autoAllocateChunkSize\"))}catch(R){return @Promise.@reject(R)}const P={buffer:d,byteOffset:0,byteLength:@getByIdDirectPrivate(_,\"autoAllocateChunkSize\"),bytesFilled:0,elementSize:1,ctor:@Uint8Array,readerType:\"default\"};@getByIdDirectPrivate(_,\"pendingPullIntos\").push(P)}const F=@readableStreamAddReadRequest(h);return @readableByteStreamControllerCallPullIfNeeded(_),F})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerRespondInternalCode = "(function (d,k){\"use strict\";let _=@getByIdDirectPrivate(d,\"pendingPullIntos\").peek(),u=@getByIdDirectPrivate(d,\"controlledReadableStream\");if(@getByIdDirectPrivate(u,\"state\")===@streamClosed){if(k!==0)@throwTypeError(\"bytesWritten is different from 0 even though stream is closed\");@readableByteStreamControllerRespondInClosedState(d,_)}else @assert(@getByIdDirectPrivate(u,\"state\")===@streamReadable),@readableByteStreamControllerRespondInReadableState(d,k,_)})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerFillDescriptorFromQueueCode = "(function (q,_){\"use strict\";const j=_.bytesFilled-_.bytesFilled%_.elementSize,v=@getByIdDirectPrivate(q,\"queue\").size<_.byteLength-_.bytesFilled\?@getByIdDirectPrivate(q,\"queue\").size:_.byteLength-_.bytesFilled,k=_.bytesFilled+v,z=k-k%_.elementSize;let w=v,E=!1;if(z>j)w=z-_.bytesFilled,E=!0;while(w>0){let G=@getByIdDirectPrivate(q,\"queue\").content.peek();const H=w<G.byteLength\?w:G.byteLength,J=_.byteOffset+_.bytesFilled;if(new @Uint8Array(_.buffer).set(new @Uint8Array(G.buffer,G.byteOffset,H),J),G.byteLength===H)@getByIdDirectPrivate(q,\"queue\").content.shift();else G.byteOffset+=H,G.byteLength-=H;@getByIdDirectPrivate(q,\"queue\").size-=H,@assert(@getByIdDirectPrivate(q,\"pendingPullIntos\").isEmpty()||@getByIdDirectPrivate(q,\"pendingPullIntos\").peek()===_),@readableByteStreamControllerInvalidateBYOBRequest(q),_.bytesFilled+=H,w-=H}if(!E)@assert(@getByIdDirectPrivate(q,\"queue\").size===0),@assert(_.bytesFilled>0),@assert(_.bytesFilled<_.elementSize);return E})\n";
...
const char* const s_eventSourceGetEventSourceCode = "(function (){\"use strict\";class B extends EventTarget{#$;#j;#w;#A;#B;#F=!1;#G=null;#J=\"\";#K=\"\";#L=\"\";#M=!0;#O=0;#Q=0;#U=0;#V=null;static#W(W){W.#H()}static#X(W,j){const w=W.data,F=w.#L\?`Last-Event-ID: ${w.#L}\\r\\n`:\"\",Q=`GET ${j.pathname}${j.search} HTTP/1.1\\r\\nHost: bun\\r\\nContent-type: text/event-stream\\r\\nContent-length: 0\\r\\n${F}\\r\\n`,X=W.write(Q);if(X!==Q.length)w.#K=Q.substring(X)}static#Y(W,j,w){for(;;){if(w>=j.length)return;let F=-1,Q=j.indexOf(\"\\r\\n\",w);const X=Q+2;if(Q>0)if(W.#O===0){const Y=parseInt(j.substring(w,Q),16);if(Y===0){W.#j=2,W.#G\?.end();return}F=X+Y}else F=j.length;else{if(W.#J.length===0){W.#J+=j.substring(w);return}F=j.length}let G=j.substring(X,F);w=F+2;let U=0,V=G.indexOf(\"\\n\\n\");if(V==-1){W.#J+=j.substring(X);return}if(W.#J.length)W.#J+=G,G=W.#J,W.#J=\"\";let J=!0;while(J){const Y=G.substring(U,V);let K,Z=\"\",A,z=0,L=-1;for(;;){let H=Y.indexOf(\"\\n\",z);if(H===-1){if(z>=Y.length)break;H=Y.length}const O=Y.substring(z,H);if(O.startsWith(\"data:\"))if(Z.length)Z+=`\\n${O.substring(5).trim()}`;else Z=O.substring(5).trim();else if(O.startsWith(\"event:\"))K=O.substring(6).trim();else if(O.startsWith(\"id:\"))A=O.substring(3).trim();else if(O.startsWith(\"retry:\")){if(L=parseInt(O.substring(6).trim(),10),@isNaN(L))L=-1}z=H+1}if(W.#L=A||\"\",L>=0)W.#U=L;if(Z||A||K)W.dispatchEvent(new MessageEvent(K||\"message\",{data:Z||\"\",origin:W.#$.origin,source:W,lastEventId:A}));if(G.length===V+2){J=!1;break}const M=G.indexOf(\"\\n\\n\",V+1);if(M===-1)break;U=V,V=M}}}static#Z={open(W){const j=W.data;if(j.#G=W,!j.#F)B.#X(W,j.#$)},handshake(W,j,w){const F=W.data;if(j)B.#X(W,F.#$);else F.#j=2,F.dispatchEvent(new ErrorEvent(\"error\",{error:w})),W.end()},data(W,j){const w=W.data;switch(w.#j){case 0:{let F=j.toString();const Q=F.indexOf(\"\\r\\n\\r\\n\");if(Q===-1){w.#J+=F;return}if(w.#J.length)w.#J+=F,F=w.#J,w.#J=\"\";const X=F.substring(0,Q),G=X.indexOf(\"\\r\\n\");if(G===-1){w.#j=2,w.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Invalid HTTP request\")})),W.end();return}const U=X.substring(0,G);if(U!==\"HTTP/1.1 200 OK\"){w.#j=2,w.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(U)})),W.end();return}let V=G+1,J=!1,Y=-1;for(;;){let Z=X.indexOf(\"\\r\\n\",V);if(Z===-1){if(V>=X.length){if(!J)w.#j=2,w.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has no MIME type and \"text/event-stream\" is required. Aborting the connection.`)})),W.end();return}Z=X.length}const A=X.substring(V+1,Z),z=A.indexOf(\":\"),L=A.substring(0,z),M=L.localeCompare(\"content-type\",@undefined,{sensitivity:\"accent\"})===0;if(V=Z+1,M)if(A.endsWith(\" text/event-stream\"))J=!0;else{w.#j=2,w.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has a MIME type that is not \"text/event-stream\". Aborting the connection.`)})),W.end();return}else if(L.localeCompare(\"content-length\",@undefined,{sensitivity:\"accent\"})===0){if(Y=parseInt(A.substring(z+1).trim(),10),@isNaN(Y)||Y<=0){w.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Content-Length is invalid. Aborting the connection.`)})),W.end();return}if(J)break}else if(L.localeCompare(\"transfer-encoding\",@undefined,{sensitivity:\"accent\"})===0){if(A.substring(z+1).trim()!==\"chunked\"){w.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Transfer-Encoding is invalid. Aborting the connection.`)})),W.end();return}if(Y=0,J)break}}w.#O=Y,w.#j=1,w.dispatchEvent(new Event(\"open\"));const K=F.substring(Q+4);if(B.#Y(w,K,0),w.#O>0){if(w.#Q+=K.length,w.#Q>=w.#O)w.#j=2,W.end()}return}case 1:if(B.#Y(w,j.toString(),2),w.#O>0){if(w.#Q+=j.byteLength,w.#Q>=w.#O)w.#j=2,W.end()}return;default:break}},drain(W){const j=W.data;if(j.#j===0){const w=j.#J;if(w.length){const F=W.write(w);if(F!==w.length)W.data.#K=w.substring(F);else W.data.#K=\"\"}}},close:B.#z,end(W){B.#z(W).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Connection closed by server\")}))},timeout(W){B.#z(W).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Timeout\")}))},binaryType:\"buffer\"};static#z(W){const j=W.data;if(j.#G=null,j.#Q=0,j.#j=2,j.#M){if(j.#V)clearTimeout(j.#V);j.#V=setTimeout(B.#W,j.#U,j)}return j}constructor(W,j=@undefined){super();const w=new URL(W);this.#F=w.protocol===\"https:\",this.#$=w,this.#j=2,process.nextTick(B.#W,this)}ref(){this.#V\?.ref(),this.#G\?.ref()}unref(){this.#V\?.unref(),this.#G\?.unref()}#H(){if(this.#j!==2)return;const W=this.#$,j=this.#F;this.#j=0,@Bun.connect({data:this,socket:B.#Z,hostname:W.hostname,port:parseInt(W.port||(j\?\"443\":\"80\"),10),tls:j\?{requestCert:!0,rejectUnauthorized:!1}:!1}).catch((w)=>{if(super.dispatchEvent(new ErrorEvent(\"error\",{error:w})),this.#M){if(this.#V)this.#V.unref\?.();this.#V=setTimeout(B.#W,1000,this)}})}get url(){return this.#$.href}get readyState(){return this.#j}close(){this.#M=!1,this.#j=2,this.#G\?.unref(),this.#G\?.end()}get onopen(){return this.#B}get onerror(){return this.#w}get onmessage(){return this.#A}set onopen(W){if(this.#B)super.removeEventListener(\"close\",this.#B);super.addEventListener(\"open\",W),this.#B=W}set onerror(W){if(this.#w)super.removeEventListener(\"error\",this.#w);super.addEventListener(\"error\",W),this.#w=W}set onmessage(W){if(this.#A)super.removeEventListener(\"message\",this.#A);super.addEventListener(\"message\",W),this.#A=W}}return Object.defineProperty(B.prototype,\"CONNECTING\",{enumerable:!0,value:0}),Object.defineProperty(B.prototype,\"OPEN\",{enumerable:!0,value:1}),Object.defineProperty(B.prototype,\"CLOSED\",{enumerable:!0,value:2}),B[Symbol.for(\"CommonJS\")]=0,B})\n";
