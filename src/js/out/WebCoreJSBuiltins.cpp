...
const char* const s_writableStreamInternalsSetUpWritableStreamDefaultWriterCode = "(function (v,g){\"use strict\";if(@isWritableStreamLocked(g))@throwTypeError(\"WritableStream is locked\");@putByIdDirectPrivate(v,\"stream\",g),@putByIdDirectPrivate(g,\"writer\",v);const h=@newPromiseCapability(@Promise),B=@newPromiseCapability(@Promise);@putByIdDirectPrivate(v,\"readyPromise\",h),@putByIdDirectPrivate(v,\"closedPromise\",B);const _=@getByIdDirectPrivate(g,\"state\");if(_===\"writable\"){if(@writableStreamCloseQueuedOrInFlight(g)||!@getByIdDirectPrivate(g,\"backpressure\"))h.@resolve.@call()}else if(_===\"erroring\")h.@reject.@call(@undefined,@getByIdDirectPrivate(g,\"storedError\")),@markPromiseAsHandled(h.@promise);else if(_===\"closed\")h.@resolve.@call(),B.@resolve.@call();else{@assert(_===\"errored\");const n=@getByIdDirectPrivate(g,\"storedError\");h.@reject.@call(@undefined,n),@markPromiseAsHandled(h.@promise),B.@reject.@call(@undefined,n),@markPromiseAsHandled(B.@promise)}})\n";
...
const char* const s_jsBufferPrototypeWriteIntBECode = "(function (d,r,c){\"use strict\";const E=this.@dataView||=new DataView(this.buffer,this.byteOffset,this.byteLength);switch(c){case 1:{E.setInt8(r,d);break}case 2:{E.setInt16(r,d,!1);break}case 3:{E.setUint16(r+1,d&65535,!1),E.setInt8(r,Math.floor(d*0.0000152587890625));break}case 4:{E.setInt32(r,d,!1);break}case 5:{E.setUint32(r+1,d|0,!1),E.setInt8(r,Math.floor(d*0.00000000023283064365386964));break}case 6:{E.setUint32(r+2,d|0,!1),E.setInt16(r,Math.floor(d*0.00000000023283064365386964),!1);break}default:@throwRangeError(\"byteLength must be >= 1 and <= 6\")}return r+c})\n";
...
const char* const s_jsBufferPrototypeBase64SliceCode = "(function (r,d){\"use strict\";return this.toString(r,d,\"base64\")})\n";
...
const char* const s_readableStreamInternalsReadableStreamPipeToWritableStreamCode = "(function (D,T,w,x,z,B){\"use strict\";if(@assert(@isReadableStream(D)),@assert(@isWritableStream(T)),@assert(!@isReadableStreamLocked(D)),@assert(!@isWritableStreamLocked(T)),@assert(B===@undefined||@isAbortSignal(B)),@getByIdDirectPrivate(D,\"underlyingByteSource\")!==@undefined)return @Promise.@reject(\"Piping to a readable bytestream is not supported\");let F={source:D,destination:T,preventAbort:x,preventCancel:z,preventClose:w,signal:B};if(F.reader=@acquireReadableStreamDefaultReader(D),F.writer=@acquireWritableStreamDefaultWriter(T),@putByIdDirectPrivate(D,\"disturbed\",!0),F.finalized=!1,F.shuttingDown=!1,F.promiseCapability=@newPromiseCapability(@Promise),F.pendingReadPromiseCapability=@newPromiseCapability(@Promise),F.pendingReadPromiseCapability.@resolve.@call(),F.pendingWritePromise=@Promise.@resolve(),B!==@undefined){const G=(H)=>{if(F.finalized)return;@pipeToShutdownWithAction(F,()=>{const _=!F.preventAbort&&@getByIdDirectPrivate(F.destination,\"state\")===\"writable\"\?@writableStreamAbort(F.destination,H):@Promise.@resolve(),E=!F.preventCancel&&@getByIdDirectPrivate(F.source,\"state\")===@streamReadable\?@readableStreamCancel(F.source,H):@Promise.@resolve();let k=@newPromiseCapability(@Promise),J=!0,K=()=>{if(J){J=!1;return}k.@resolve.@call()},L=(q)=>{k.@reject.@call(@undefined,q)};return _.@then(K,L),E.@then(K,L),k.@promise},H)};if(@whenSignalAborted(B,G))return F.promiseCapability.@promise}return @pipeToErrorsMustBePropagatedForward(F),@pipeToErrorsMustBePropagatedBackward(F),@pipeToClosingMustBePropagatedForward(F),@pipeToClosingMustBePropagatedBackward(F),@pipeToLoop(F),F.promiseCapability.@promise})\n";
...
const char* const s_readableStreamInternalsReadableStreamTeeCode = "(function (i,g){\"use strict\";@assert(@isReadableStream(i)),@assert(typeof g===\"boolean\");var k=@getByIdDirectPrivate(i,\"start\");if(k)@putByIdDirectPrivate(i,\"start\",@undefined),k();const j=new @ReadableStreamDefaultReader(i),q={closedOrErrored:!1,canceled1:!1,canceled2:!1,reason1:@undefined,reason2:@undefined};q.cancelPromiseCapability=@newPromiseCapability(@Promise);const v=@readableStreamTeePullFunction(q,j,g),_={};@putByIdDirectPrivate(_,\"pull\",v),@putByIdDirectPrivate(_,\"cancel\",@readableStreamTeeBranch1CancelFunction(q,i));const w={};@putByIdDirectPrivate(w,\"pull\",v),@putByIdDirectPrivate(w,\"cancel\",@readableStreamTeeBranch2CancelFunction(q,i));const x=new @ReadableStream(_),f=new @ReadableStream(w);return @getByIdDirectPrivate(j,\"closedPromiseCapability\").@promise.@then(@undefined,function(y){if(q.closedOrErrored)return;if(@readableStreamDefaultControllerError(x.@readableStreamController,y),@readableStreamDefaultControllerError(f.@readableStreamController,y),q.closedOrErrored=!0,!q.canceled1||!q.canceled2)q.cancelPromiseCapability.@resolve.@call()}),q.branch1=x,q.branch2=f,[x,f]})\n";
...
const char* const s_readableStreamInternalsReadDirectStreamCode = "(function (_,j,z){\"use strict\";@putByIdDirectPrivate(_,\"underlyingSource\",@undefined),@putByIdDirectPrivate(_,\"start\",@undefined);function q(A,w){var B=z\?.cancel;if(B){try{var x=B.@apply(z,w);if(@isPromise(x))@markPromiseAsHandled(x)}catch(C){}z=@undefined}if(A)if(@putByIdDirectPrivate(A,\"readableStreamController\",@undefined),@putByIdDirectPrivate(A,\"reader\",@undefined),w)@putByIdDirectPrivate(A,\"state\",@streamErrored),@putByIdDirectPrivate(A,\"storedError\",w);else @putByIdDirectPrivate(A,\"state\",@streamClosed)}if(!z.pull){q(_);return}if(!@isCallable(z.pull))q(_),@throwTypeError(\"pull is not a function\");@putByIdDirectPrivate(_,\"readableStreamController\",j);const f=@getByIdDirectPrivate(_,\"highWaterMark\");j.start({highWaterMark:!f||f<64\?64:f}),@startDirectStream.@call(j,_,z.pull,q),@putByIdDirectPrivate(_,\"reader\",{});var v=z.pull(j);if(j=@undefined,v&&@isPromise(v))return v.@then(()=>{})})\n";
...
const char* const s_readableStreamInternalsReadStreamIntoSinkCode = "(async function (c,f,I){\"use strict\";var D=!1,P=!1;try{var E=c.getReader(),q=E.readMany();if(q&&@isPromise(q))q=await q;if(q.done)return D=!0,f.end();var F=q.value.length;const J=@getByIdDirectPrivate(c,\"highWaterMark\");if(I)@startDirectStream.@call(f,c,@undefined,()=>!P&&@markPromiseAsHandled(c.cancel()));f.start({highWaterMark:J||0});for(var K=0,G=q.value,x=q.value.length;K<x;K++)f.write(G[K]);var _=@getByIdDirectPrivate(c,\"state\");if(_===@streamClosed)return D=!0,f.end();while(!0){var{value:z,done:H}=await E.read();if(H)return D=!0,f.end();f.write(z)}}catch(J){P=!0;try{E=@undefined;const B=c.cancel(J);@markPromiseAsHandled(B)}catch(B){}if(f&&!D){D=!0;try{f.close(J)}catch(B){throw new globalThis.AggregateError([J,B])}}throw J}finally{if(E){try{E.releaseLock()}catch(B){}E=@undefined}f=@undefined;var _=@getByIdDirectPrivate(c,\"state\");if(c){var A=@getByIdDirectPrivate(c,\"readableStreamController\");if(A){if(@getByIdDirectPrivate(A,\"underlyingSource\"))@putByIdDirectPrivate(A,\"underlyingSource\",@undefined);if(@getByIdDirectPrivate(A,\"controlledReadableStream\"))@putByIdDirectPrivate(A,\"controlledReadableStream\",@undefined);if(@putByIdDirectPrivate(c,\"readableStreamController\",null),@getByIdDirectPrivate(c,\"underlyingSource\"))@putByIdDirectPrivate(c,\"underlyingSource\",@undefined);A=@undefined}if(!P&&_!==@streamClosed&&_!==@streamErrored)@readableStreamClose(c);c=@undefined}}})\n";
...
const char* const s_readableStreamInternalsCreateTextStreamCode = "(function (_){\"use strict\";var q,z=[],v=!1,j=!1,w=\"\",x=@toLength(0),A=@newPromiseCapability(@Promise),C=!1;return q={start(){},write(E){if(typeof E===\"string\"){var F=@toLength(E.length);if(F>0)w+=E,v=!0,x+=F;return F}if(!E||!(@ArrayBuffer.@isView(E)||E instanceof @ArrayBuffer))@throwTypeError(\"Expected text, ArrayBuffer or ArrayBufferView\");const G=@toLength(E.byteLength);if(G>0)if(j=!0,w.length>0)@arrayPush(z,w,E),w=\"\";else @arrayPush(z,E);return x+=G,G},flush(){return 0},end(){if(C)return\"\";return q.fulfill()},fulfill(){C=!0;const E=q.finishInternal();return @fulfillPromise(A.@promise,E),E},finishInternal(){if(!v&&!j)return\"\";if(v&&!j)return w;if(j&&!v)return new globalThis.TextDecoder().decode(@Bun.concatArrayBuffers(z));var E=new @Bun.ArrayBufferSink;E.start({highWaterMark:x,asUint8Array:!0});for(let F of z)E.write(F);if(z.length=0,w.length>0)E.write(w),w=\"\";return new globalThis.TextDecoder().decode(E.end())},close(){try{if(!C)C=!0,q.fulfill()}catch(E){}}},[q,A]})\n";
...
const char* const s_readableStreamInternalsInitializeArrayStreamCode = "(function (t,d){\"use strict\";var j=[],m=@newPromiseCapability(@Promise),q=!1;function p(){return q=!0,m.@resolve.@call(@undefined,j),j}var _={start(){},write(b){return @arrayPush(j,b),b.byteLength||b.length},flush(){return 0},end(){if(q)return[];return p()},close(){if(!q)p()}},v={@underlyingSource:t,@pull:@onPullDirectStream,@controlledReadableStream:this,@sink:_,close:@onCloseDirectStream,write:_.write,error:@handleDirectStreamError,end:@onCloseDirectStream,@close:@onCloseDirectStream,flush:@onFlushDirectStream,_pendingRead:@undefined,_deferClose:0,_deferFlush:0,_deferCloseReason:@undefined,_handleError:@undefined};return @putByIdDirectPrivate(this,\"readableStreamController\",v),@putByIdDirectPrivate(this,\"underlyingSource\",@undefined),@putByIdDirectPrivate(this,\"start\",@undefined),m})\n";
...
const char* const s_readableStreamInternalsReadableStreamToArrayBufferDirectCode = "(function (q,w){\"use strict\";var x=new @Bun.ArrayBufferSink;@putByIdDirectPrivate(q,\"underlyingSource\",@undefined);var z=@getByIdDirectPrivate(q,\"highWaterMark\");x.start(z\?{highWaterMark:z}:{});var O=@newPromiseCapability(@Promise),D=!1,j=w.pull,F=w.close,G={start(){},close(B){if(!D){if(D=!0,F)F();@fulfillPromise(O.@promise,x.end())}},end(){if(!D){if(D=!0,F)F();@fulfillPromise(O.@promise,x.end())}},flush(){return 0},write:x.write.bind(x)},v=!1;try{const B=j(G);if(B&&@isObject(B)&&@isPromise(B))return async function(_,A,C){while(!D)await C(_);return await A}(G,promise,j);return O.@promise}catch(B){return v=!0,@readableStreamError(q,B),@Promise.@reject(B)}finally{if(!v&&q)@readableStreamClose(q);G=F=x=j=q=@undefined}})\n";
...
const char* const s_readableStreamInternalsReadableStreamDefineLazyIteratorsCode = "(function (h){\"use strict\";var k=globalThis.Symbol.asyncIterator,g=async function*q(z,B){var D=z.getReader(),F;try{while(!0){var w,x;const G=D.readMany();if(@isPromise(G))({done:w,value:x}=await G);else({done:w,value:x}=G);if(w)return;yield*x}}catch(G){F=G}finally{if(D.releaseLock(),!B)z.cancel(F);if(F)throw F}},j=function q(){return g(this,!1)},i=function q({preventCancel:z=!1}={preventCancel:!1}){return g(this,z)};return @Object.@defineProperty(h,k,{value:j}),@Object.@defineProperty(h,\"values\",{value:i}),h})\n";
...
const char* const s_eventStreamGetEventStreamCode = "(function (){\"use strict\";class D extends @ReadableStream{#$;@contentType=\"text/event-stream\";constructor(R){var _=[],j=!1;super({type:\"direct\",pull:(k)=>{if(this.#$=k,_.length){for(let g of _)if(k.write(g),g===null){k.close(),this.#$=null;return}k.flush()}R\?.start\?.(this),j=!0},cancel:()=>{if(j)R\?.cancel\?.(this);this.#$=null}});this.#$={write:(k)=>_.push(k),flush:()=>{},close:()=>{_.push(null)}}}setReconnectionTime(R){var _=this.#$;if(!_)throw new Error(\"EventStream is closed\");_.write(\"retry:\"+R+\"\\n\\n\")}send(R,_,j){var k=this.#$;if(!k)throw new Error(\"EventStream is closed\");if(!_)_=R,R=@undefined;else if(R===\"message\")R=@undefined;if(_===@undefined)@throwTypeError(\"EventStream.send() requires a data argument\");if(j!==@undefined)this.#D(k,R,_,j);else this.#g(k,R,_);k.flush()}#g(R,_,j){if(_)R.write(\"event:\"+_+\"\\n\");if(typeof j===\"string\")R.write(\"data:\"+j.replace(/\\n/g,\"\\ndata:\")+\"\\n\\n\");else if(@isTypedArrayView(j)||j instanceof @ArrayBuffer)R.write(\"data:\"),R.write(j),R.write(\"\\n\\n\");else R.write(\"data:\"+JSON.stringify(j)+\"\\n\\n\")}#D(R,_,j,k){if(_)R.write(\"event:\"+_+\"\\n\");if(typeof j===\"string\")R.write(\"data:\"+j.replace(/\\n/g,\"\\ndata:\")+\"\\n\");else if(j instanceof @Uint8Array)R.write(\"data:\"),R.write(j),R.write(\"\\n\");else R.write(\"data:\"+JSON.stringify(j)+\"\\n\");if(k===null)R.write(\"id\\n\");else R.write(\"id:\"+k+\"\\n\")}close(){this.#$\?.close()}}return Object.defineProperty(D,\"name\",{value:\"EventStream\"}),D})\n";
...
const char* const s_readableStreamDefaultReaderReadManyCode = "(function (){\"use strict\";if(!@isReadableStreamDefaultReader(this))@throwTypeError(\"ReadableStreamDefaultReader.readMany() should not be called directly\");const U=@getByIdDirectPrivate(this,\"ownerReadableStream\");if(!U)@throwTypeError(\"readMany() called on a reader owned by no readable stream\");const W=@getByIdDirectPrivate(U,\"state\");if(@putByIdDirectPrivate(U,\"disturbed\",!0),W===@streamClosed)return{value:[],size:0,done:!0};else if(W===@streamErrored)throw @getByIdDirectPrivate(U,\"storedError\");var d=@getByIdDirectPrivate(U,\"readableStreamController\"),I=@getByIdDirectPrivate(d,\"queue\");if(!I)return d.@pull(d).@then(function({done:Q,value:_}){return Q\?{done:!0,value:[],size:0}:{value:[_],size:1,done:!1}});const H=I.content;var K=I.size,N=H.toArray(!1),x=N.length;if(x>0){var B=@newArrayWithSize(x);if(@isReadableByteStreamController(d)){{const Q=N[0];if(!(@ArrayBuffer.@isView(Q)||Q instanceof @ArrayBuffer))@putByValDirect(B,0,new @Uint8Array(Q.buffer,Q.byteOffset,Q.byteLength));else @putByValDirect(B,0,Q)}for(var k=1;k<x;k++){const Q=N[k];if(!(@ArrayBuffer.@isView(Q)||Q instanceof @ArrayBuffer))@putByValDirect(B,k,new @Uint8Array(Q.buffer,Q.byteOffset,Q.byteLength));else @putByValDirect(B,k,Q)}}else{@putByValDirect(B,0,N[0].value);for(var k=1;k<x;k++)@putByValDirect(B,k,N[k].value)}if(@resetQueue(@getByIdDirectPrivate(d,\"queue\")),@getByIdDirectPrivate(d,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(d,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(d))@readableStreamDefaultControllerCallPullIfNeeded(d);else if(@isReadableByteStreamController(d))@readableByteStreamControllerCallPullIfNeeded(d);return{value:B,size:K,done:!1}}var D=(Q)=>{if(Q.done)return{value:[],size:0,done:!0};var _=@getByIdDirectPrivate(U,\"readableStreamController\"),w=@getByIdDirectPrivate(_,\"queue\"),F=[Q.value].concat(w.content.toArray(!1)),A=F.length;if(@isReadableByteStreamController(_))for(var C=0;C<A;C++){const j=F[C];if(!(@ArrayBuffer.@isView(j)||j instanceof @ArrayBuffer)){const{buffer:E,byteOffset:J,byteLength:T}=j;@putByValDirect(F,C,new @Uint8Array(E,J,T))}}else for(var C=1;C<A;C++)@putByValDirect(F,C,F[C].value);var G=w.size;if(@resetQueue(w),@getByIdDirectPrivate(_,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(_,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(_))@readableStreamDefaultControllerCallPullIfNeeded(_);else if(@isReadableByteStreamController(_))@readableByteStreamControllerCallPullIfNeeded(_);return{value:F,size:G,done:!1}},S=d.@pull(d);if(S&&@isPromise(S))return S.@then(D);return D(S)})\n";
...
const char* const s_importMetaObjectLoadCJS2ESMCode = "(function (w){\"use strict\";var B=@Loader,x=@createFIFO(),I=w;while(I){var _=B.registry.@get(I);if((_\?.state\?\?0)<=@ModuleFetch)@fulfillModuleSync(I),_=B.registry.@get(I);var L=@getPromiseInternalField(_.fetch,@promiseFieldReactionsOrResult),D=B.parseModule(I,L),Q=_.module;if(D&&@isPromise(D)){var T=@getPromiseInternalField(D,@promiseFieldReactionsOrResult),J=@getPromiseInternalField(D,@promiseFieldFlags),U=J&@promiseStateMask;if(U===@promiseStatePending||T&&@isPromise(T))@throwTypeError(`require() async module \"${I}\" is unsupported. use \"await import()\" instead.`);else if(U===@promiseStateRejected){if(!T\?.message)@throwTypeError(`${T+\"\"\?T:\"An error occurred\"} occurred while parsing module \\\"${I}\\\"`);throw T}_.module=Q=T}else if(D&&!Q)_.module=Q=D;@setStateToMax(_,@ModuleLink);var V=Q.dependenciesMap,W=B.requestedModules(Q),X=@newArrayWithSize(W.length);for(var Y=0,Z=W.length;Y<Z;++Y){var F=W[Y],G=F[0]===\"/\"\?F:B.resolve(F,I),z=B.ensureRegistered(G);if(z.state<@ModuleLink)x.push(G);@putByValDirect(X,Y,z),V.@set(F,z)}_.dependencies=X,_.instantiate=@Promise.resolve(_),_.satisfy=@Promise.resolve(_),I=x.shift();while(I&&(B.registry.@get(I)\?.state\?\?@ModuleFetch)>=@ModuleLink)I=x.shift()}var H=B.linkAndEvaluateModule(w,@undefined);if(H&&@isPromise(H))@throwTypeError(`require() async module \\\"${w}\\\" is unsupported. use \"await import()\" instead.`);return B.registry.@get(w)})\n";
...
const char* const s_importMetaObjectInternalRequireCode = "(function (_){\"use strict\";var g=@requireMap.@get(_);const k=_.substring(_.length-5);if(g)return g.exports;if(k===\".json\"){var n=globalThis[Symbol.for(\"_fs\")]||=@Bun.fs(),q=JSON.parse(n.readFileSync(_,\"utf8\"));return @requireMap.@set(_,@createCommonJSModule(_,q,!0)),q}else if(k===\".node\"){const b=@createCommonJSModule(_,{},!0);return process.dlopen(b,_),@requireMap.@set(_,b),b.exports}else if(k===\".toml\"){var n=globalThis[Symbol.for(\"_fs\")]||=@Bun.fs(),q=@Bun.TOML.parse(n.readFileSync(_,\"utf8\"));return @requireMap.@set(_,@createCommonJSModule(_,q,!0)),q}else{var q=@requireESM(_);const j=@requireMap.@get(_);if(j)return j.exports;var v=q\?.default;if(v\?.[@commonJSSymbol]===0)q=v;return @requireMap.@set(_,@createCommonJSModule(_,q,!0)),q}})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerCommitDescriptorCode = "(function (_,b){\"use strict\";@assert(@getByIdDirectPrivate(_,\"state\")!==@streamErrored);let h=!1;if(@getByIdDirectPrivate(_,\"state\")===@streamClosed)@assert(!b.bytesFilled),h=!0;let g=@readableByteStreamControllerConvertDescriptor(b);if(b.readerType===\"default\")@readableStreamFulfillReadRequest(_,g,h);else @assert(b.readerType===\"byob\"),@readableStreamFulfillReadIntoRequest(_,g,h)})\n";
...
const char* const s_eventSourceGetEventSourceCode = "(function (){\"use strict\";class j extends EventTarget{#Q;#G;#M;#J;#K;#$=!1;#w=null;#U=\"\";#A=\"\";#L=\"\";#O=!0;#V=0;#W=0;#X=0;#Y=null;static#Z(w){w.#H()}static#B(w,V){const W=w.data,X=W.#L\?`Last-Event-ID: ${W.#L}\\r\\n`:\"\",A=`GET ${V.pathname}${V.search} HTTP/1.1\\r\\nHost: bun\\r\\nContent-type: text/event-stream\\r\\nContent-length: 0\\r\\n${X}\\r\\n`,B=w.write(A);if(B!==A.length)W.#A=A.substring(B)}static#j(w,V,W){for(;;){if(W>=V.length)return;let X=-1,A=V.indexOf(\"\\r\\n\",W);const B=A+2;if(A>0)if(w.#V===0){const L=parseInt(V.substring(W,A),16);if(L===0){w.#G=2,w.#w\?.end();return}X=B+L}else X=V.length;else{if(w.#U.length===0){w.#U+=V.substring(W);return}X=V.length}let Y=V.substring(B,X);W=X+2;let J=0,K=Y.indexOf(\"\\n\\n\");if(K==-1){w.#U+=V.substring(B);return}if(w.#U.length)w.#U+=Y,Y=w.#U,w.#U=\"\";let Z=!0;while(Z){const L=Y.substring(J,K);let F,M=\"\",z,O=0,H=-1;for(;;){let G=L.indexOf(\"\\n\",O);if(G===-1){if(O>=L.length)break;G=L.length}const U=L.substring(O,G);if(U.startsWith(\"data:\"))if(M.length)M+=`\\n${U.substring(5).trim()}`;else M=U.substring(5).trim();else if(U.startsWith(\"event:\"))F=U.substring(6).trim();else if(U.startsWith(\"id:\"))z=U.substring(3).trim();else if(U.startsWith(\"retry:\")){if(H=parseInt(U.substring(6).trim(),10),@isNaN(H))H=-1}O=G+1}if(w.#L=z||\"\",H>=0)w.#X=H;if(M||z||F)w.dispatchEvent(new MessageEvent(F||\"message\",{data:M||\"\",origin:w.#Q.origin,source:w,lastEventId:z}));if(Y.length===K+2){Z=!1;break}const Q=Y.indexOf(\"\\n\\n\",K+1);if(Q===-1)break;J=K,K=Q}}}static#F={open(w){const V=w.data;if(V.#w=w,!V.#$)j.#B(w,V.#Q)},handshake(w,V,W){const X=w.data;if(V)j.#B(w,X.#Q);else X.#G=2,X.dispatchEvent(new ErrorEvent(\"error\",{error:W})),w.end()},data(w,V){const W=w.data;switch(W.#G){case 0:{let X=V.toString();const A=X.indexOf(\"\\r\\n\\r\\n\");if(A===-1){W.#U+=X;return}if(W.#U.length)W.#U+=X,X=W.#U,W.#U=\"\";const B=X.substring(0,A),Y=B.indexOf(\"\\r\\n\");if(Y===-1){W.#G=2,W.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Invalid HTTP request\")})),w.end();return}const J=B.substring(0,Y);if(J!==\"HTTP/1.1 200 OK\"){W.#G=2,W.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(J)})),w.end();return}let K=Y+1,Z=!1,L=-1;for(;;){let M=B.indexOf(\"\\r\\n\",K);if(M===-1){if(K>=B.length){if(!Z)W.#G=2,W.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has no MIME type and \"text/event-stream\" is required. Aborting the connection.`)})),w.end();return}M=B.length}const z=B.substring(K+1,M),O=z.indexOf(\":\"),H=z.substring(0,O),Q=H.localeCompare(\"content-type\",@undefined,{sensitivity:\"accent\"})===0;if(K=M+1,Q)if(z.endsWith(\" text/event-stream\"))Z=!0;else{W.#G=2,W.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has a MIME type that is not \"text/event-stream\". Aborting the connection.`)})),w.end();return}else if(H.localeCompare(\"content-length\",@undefined,{sensitivity:\"accent\"})===0){if(L=parseInt(z.substring(O+1).trim(),10),@isNaN(L)||L<=0){W.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Content-Length is invalid. Aborting the connection.`)})),w.end();return}if(Z)break}else if(H.localeCompare(\"transfer-encoding\",@undefined,{sensitivity:\"accent\"})===0){if(z.substring(O+1).trim()!==\"chunked\"){W.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Transfer-Encoding is invalid. Aborting the connection.`)})),w.end();return}if(L=0,Z)break}}W.#V=L,W.#G=1,W.dispatchEvent(new Event(\"open\"));const F=X.substring(A+4);if(j.#j(W,F,0),W.#V>0){if(W.#W+=F.length,W.#W>=W.#V)W.#G=2,w.end()}return}case 1:if(j.#j(W,V.toString(),2),W.#V>0){if(W.#W+=V.byteLength,W.#W>=W.#V)W.#G=2,w.end()}return;default:break}},drain(w){const V=w.data;if(V.#G===0){const W=V.#U;if(W.length){const X=w.write(W);if(X!==W.length)w.data.#A=W.substring(X);else w.data.#A=\"\"}}},close:j.#z,end(w){j.#z(w).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Connection closed by server\")}))},timeout(w){j.#z(w).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Timeout\")}))},binaryType:\"buffer\"};static#z(w){const V=w.data;if(V.#w=null,V.#W=0,V.#G=2,V.#O){if(V.#Y)clearTimeout(V.#Y);V.#Y=setTimeout(j.#Z,V.#X,V)}return V}constructor(w,V=@undefined){super();const W=new URL(w);this.#$=W.protocol===\"https:\",this.#Q=W,this.#G=2,process.nextTick(j.#Z,this)}ref(){this.#Y\?.ref(),this.#w\?.ref()}unref(){this.#Y\?.unref(),this.#w\?.unref()}#H(){if(this.#G!==2)return;const w=this.#Q,V=this.#$;this.#G=0,@Bun.connect({data:this,socket:j.#F,hostname:w.hostname,port:parseInt(w.port||(V\?\"443\":\"80\"),10),tls:V\?{requestCert:!0,rejectUnauthorized:!1}:!1}).catch((W)=>{if(super.dispatchEvent(new ErrorEvent(\"error\",{error:W})),this.#O){if(this.#Y)this.#Y.unref\?.();this.#Y=setTimeout(j.#Z,1000,this)}})}get url(){return this.#Q.href}get readyState(){return this.#G}close(){this.#O=!1,this.#G=2,this.#w\?.unref(),this.#w\?.end()}get onopen(){return this.#K}get onerror(){return this.#M}get onmessage(){return this.#J}set onopen(w){if(this.#K)super.removeEventListener(\"close\",this.#K);super.addEventListener(\"open\",w),this.#K=w}set onerror(w){if(this.#M)super.removeEventListener(\"error\",this.#M);super.addEventListener(\"error\",w),this.#M=w}set onmessage(w){if(this.#J)super.removeEventListener(\"message\",this.#J);super.addEventListener(\"message\",w),this.#J=w}}return Object.defineProperty(j.prototype,\"CONNECTING\",{enumerable:!0,value:0}),Object.defineProperty(j.prototype,\"OPEN\",{enumerable:!0,value:1}),Object.defineProperty(j.prototype,\"CLOSED\",{enumerable:!0,value:2}),j[Symbol.for(\"CommonJS\")]=0,j})\n";
