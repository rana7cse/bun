...
const char* const s_writableStreamInternalsCreateWritableStreamCode = "(function (u,p,f,N,j,q){\"use strict\";@assert(typeof j===\"number\"&&!@isNaN(j)&&j>=0);const d={};@initializeWritableStreamSlots(d,{});const _=new @WritableStreamDefaultController;return @setUpWritableStreamDefaultController(d,_,u,p,f,N,j,q),@createWritableStreamFromInternal(d)})\n";
...
const char* const s_transformStreamInternalsTransformStreamErrorCode = "(function (f,i){\"use strict\";const _=@getByIdDirectPrivate(f,\"readable\"),n=@getByIdDirectPrivate(_,\"readableStreamController\");@readableStreamDefaultControllerError(n,i),@transformStreamErrorWritableAndUnblockWrite(f,i)})\n";
...
const char* const s_transformStreamInternalsTransformStreamDefaultControllerEnqueueCode = "(function (_,f){\"use strict\";const g=@getByIdDirectPrivate(_,\"stream\"),W=@getByIdDirectPrivate(g,\"readable\"),i=@getByIdDirectPrivate(W,\"readableStreamController\");if(@assert(i!==@undefined),!@readableStreamDefaultControllerCanCloseOrEnqueue(i))@throwTypeError(\"TransformStream.readable cannot close or enqueue\");try{@readableStreamDefaultControllerEnqueue(i,f)}catch(S){throw @transformStreamErrorWritableAndUnblockWrite(g,S),@getByIdDirectPrivate(W,\"storedError\")}const j=!@readableStreamDefaultControllerShouldCallPull(i);if(j!==@getByIdDirectPrivate(g,\"backpressure\"))@assert(j),@transformStreamSetBackpressure(g,!0)})\n";
...
const char* const s_transformStreamInternalsTransformStreamDefaultSinkWriteAlgorithmCode = "(function (_,v){\"use strict\";const q=@getByIdDirectPrivate(_,\"internalWritable\");@assert(@getByIdDirectPrivate(q,\"state\")===\"writable\");const f=@getByIdDirectPrivate(_,\"controller\");if(@getByIdDirectPrivate(_,\"backpressure\")){const x=@newPromiseCapability(@Promise),z=@getByIdDirectPrivate(_,\"backpressureChangePromise\");return @assert(z!==@undefined),z.@promise.@then(()=>{const d=@getByIdDirectPrivate(q,\"state\");if(d===\"erroring\"){x.@reject.@call(@undefined,@getByIdDirectPrivate(q,\"storedError\"));return}@assert(d===\"writable\"),@transformStreamDefaultControllerPerformTransform(f,v).@then(()=>{x.@resolve()},(j)=>{x.@reject.@call(@undefined,j)})},(d)=>{x.@reject.@call(@undefined,d)}),x.@promise}return @transformStreamDefaultControllerPerformTransform(f,v)})\n";
...
const char* const s_jsBufferPrototypeWriteUIntBECode = "(function (d,p,r){\"use strict\";const _=this.@dataView||=new DataView(this.buffer,this.byteOffset,this.byteLength);switch(r){case 1:{_.setUint8(p,d);break}case 2:{_.setUint16(p,d,!1);break}case 3:{_.setUint16(p+1,d&65535,!1),_.setUint8(p,Math.floor(d*0.0000152587890625));break}case 4:{_.setUint32(p,d,!1);break}case 5:{_.setUint32(p+1,d|0,!1),_.setUint8(p,Math.floor(d*0.00000000023283064365386964));break}case 6:{_.setUint32(p+2,d|0,!1),_.setUint16(p,Math.floor(d*0.00000000023283064365386964),!1);break}default:@throwRangeError(\"byteLength must be >= 1 and <= 6\")}return p+r})\n";
...
const char* const s_jsBufferPrototypeSliceCode = "(function (p,q){\"use strict\";var{buffer:c,byteOffset:i,byteLength:m}=this;function v(z,k){if(z=@trunc(z),z===0||@isNaN(z))return 0;else if(z<0)return z+=k,z>0\?z:0;else return z<k\?z:k}var w=v(p,m),x=q!==@undefined\?v(q,m):m;return new @Buffer(c,i+w,x>w\?x-w:0)})\n";
...
const char* const s_readableStreamInternalsReadableStreamPipeToWritableStreamCode = "(function (D,E,q,x,z,B){\"use strict\";if(@assert(@isReadableStream(D)),@assert(@isWritableStream(E)),@assert(!@isReadableStreamLocked(D)),@assert(!@isWritableStreamLocked(E)),@assert(B===@undefined||@isAbortSignal(B)),@getByIdDirectPrivate(D,\"underlyingByteSource\")!==@undefined)return @Promise.@reject(\"Piping to a readable bytestream is not supported\");let F={source:D,destination:E,preventAbort:x,preventCancel:z,preventClose:q,signal:B};if(F.reader=@acquireReadableStreamDefaultReader(D),F.writer=@acquireWritableStreamDefaultWriter(E),@putByIdDirectPrivate(D,\"disturbed\",!0),F.finalized=!1,F.shuttingDown=!1,F.promiseCapability=@newPromiseCapability(@Promise),F.pendingReadPromiseCapability=@newPromiseCapability(@Promise),F.pendingReadPromiseCapability.@resolve.@call(),F.pendingWritePromise=@Promise.@resolve(),B!==@undefined){const G=(H)=>{if(F.finalized)return;@pipeToShutdownWithAction(F,()=>{const _=!F.preventAbort&&@getByIdDirectPrivate(F.destination,\"state\")===\"writable\"\?@writableStreamAbort(F.destination,H):@Promise.@resolve(),T=!F.preventCancel&&@getByIdDirectPrivate(F.source,\"state\")===@streamReadable\?@readableStreamCancel(F.source,H):@Promise.@resolve();let w=@newPromiseCapability(@Promise),J=!0,K=()=>{if(J){J=!1;return}w.@resolve.@call()},k=(L)=>{w.@reject.@call(@undefined,L)};return _.@then(K,k),T.@then(K,k),w.@promise},H)};if(@whenSignalAborted(B,G))return F.promiseCapability.@promise}return @pipeToErrorsMustBePropagatedForward(F),@pipeToErrorsMustBePropagatedBackward(F),@pipeToClosingMustBePropagatedForward(F),@pipeToClosingMustBePropagatedBackward(F),@pipeToLoop(F),F.promiseCapability.@promise})\n";
...
const char* const s_readableStreamInternalsOnPullDirectStreamCode = "(function (g){\"use strict\";var h=g.@controlledReadableStream;if(!h||@getByIdDirectPrivate(h,\"state\")!==@streamReadable)return;if(g._deferClose===-1)return;g._deferClose=-1,g._deferFlush=-1;var i,y;try{var b=g.@underlyingSource.pull(g);if(b&&@isPromise(b)){if(g._handleError===@undefined)g._handleError=@handleDirectStreamErrorReject.bind(g);@Promise.prototype.catch.@call(b,g._handleError)}}catch(k){return @handleDirectStreamErrorReject.@call(g,k)}finally{i=g._deferClose,y=g._deferFlush,g._deferFlush=g._deferClose=0}var _;if(g._pendingRead===@undefined)g._pendingRead=_=@newPromise();else _=@readableStreamAddReadRequest(h);if(i===1){var j=g._deferCloseReason;return g._deferCloseReason=@undefined,@onCloseDirectStream.@call(g,j),_}if(y===1)@onFlushDirectStream.@call(g);return _})\n";
...
const char* const s_readableStreamInternalsLazyLoadStreamCode = "(function (f,b){\"use strict\";var D=@getByIdDirectPrivate(f,\"bunNativeType\"),E=@getByIdDirectPrivate(f,\"bunNativePtr\"),F=@lazyStreamPrototypeMap.@get(D);if(F===@undefined){let x=function(Y){var{c:_,v:p}=this;this.c=@undefined,this.v=@undefined,M(Y,_,p)},y=function(Y){try{Y.close()}catch(_){globalThis.reportError(_)}},X=function(Y,_,p,z){z[0]=!1;var A;try{A=j(Y,p,z)}catch(W){return _.error(W)}return M(A,_,p)};var U=x,Z=y,Q=X,[j,G,H,I,J,m,K]=@lazyLoad(D),L=[!1],M;M=function Y(_,p,z){if(_&&@isPromise(_))return _.then(x.bind({c:p,v:z}),(A)=>p.error(A));else if(typeof _===\"number\")if(z&&z.byteLength===_&&z.buffer===p.byobRequest\?.view\?.buffer)p.byobRequest.respondWithNewView(z);else p.byobRequest.respond(_);else if(_.constructor===@Uint8Array)p.enqueue(_);if(L[0]||_===!1)@enqueueJob(y,p),L[0]=!1};const B=J\?new FinalizationRegistry(J):null;F=class Y{constructor(_,p,z){if(this.#f=_,this.#b={},this.pull=this.#j.bind(this),this.cancel=this.#m.bind(this),this.autoAllocateChunkSize=p,z!==@undefined)this.start=(A)=>{A.enqueue(z)};if(B)B.register(this,_,this.#b)}#b;pull;cancel;start;#f;type=\"bytes\";autoAllocateChunkSize=0;static startSync=G;#j(_){var p=this.#f;if(!p){_.close();return}X(p,_,_.byobRequest.view,L)}#m(_){var p=this.#f;B&&B.unregister(this.#b),m&&m(p,!1),H(p,_)}static deinit=J;static drain=K},@lazyStreamPrototypeMap.@set(D,F)}const N=F.startSync(E,b);var q;const{drain:O,deinit:P}=F;if(O)q=O(E);if(N===0){if(J&&E&&@enqueueJob(J,E),(q\?.byteLength\?\?0)>0)return{start(x){x.enqueue(q),x.close()},type:\"bytes\"};return{start(x){x.close()},type:\"bytes\"}}return new F(E,N,q)})\n";
...
const char* const s_readableStreamInternalsReadableStreamIntoArrayCode = "(function (_){\"use strict\";var j=_.getReader(),f=j.readMany();async function g(p){if(p.done)return[];var b=p.value||[];while(!0){var q=await j.read();if(q.done)break;b=b.concat(q.value)}return b}if(f&&@isPromise(f))return f.@then(g);return g(f)})\n";
...
const char* const s_readableStreamInternalsReadableStreamToArrayBufferDirectCode = "(function (_,x){\"use strict\";var A=new @Bun.ArrayBufferSink;@putByIdDirectPrivate(_,\"underlyingSource\",@undefined);var O=@getByIdDirectPrivate(_,\"highWaterMark\");A.start(O\?{highWaterMark:O}:{});var v=@newPromiseCapability(@Promise),z=!1,B=x.pull,C=x.close,D={start(){},close(F){if(!z){if(z=!0,C)C();@fulfillPromise(v.@promise,A.end())}},end(){if(!z){if(z=!0,C)C();@fulfillPromise(v.@promise,A.end())}},flush(){return 0},write:A.write.bind(A)},j=!1;try{const F=B(D);if(F&&@isObject(F)&&@isPromise(F))return async function(G,q,w){while(!z)await w(G);return await q}(D,promise,B);return v.@promise}catch(F){return j=!0,@readableStreamError(_,F),@Promise.@reject(F)}finally{if(!j&&_)@readableStreamClose(_);D=C=A=B=_=@undefined}})\n";
...
const char* const s_readableStreamDefaultReaderReadManyCode = "(function (){\"use strict\";if(!@isReadableStreamDefaultReader(this))@throwTypeError(\"ReadableStreamDefaultReader.readMany() should not be called directly\");const T=@getByIdDirectPrivate(this,\"ownerReadableStream\");if(!T)@throwTypeError(\"readMany() called on a reader owned by no readable stream\");const C=@getByIdDirectPrivate(T,\"state\");if(@putByIdDirectPrivate(T,\"disturbed\",!0),C===@streamClosed)return{value:[],size:0,done:!0};else if(C===@streamErrored)throw @getByIdDirectPrivate(T,\"storedError\");var K=@getByIdDirectPrivate(T,\"readableStreamController\"),E=@getByIdDirectPrivate(K,\"queue\");if(!E)return K.@pull(K).@then(function({done:Q,value:w}){return Q\?{done:!0,value:[],size:0}:{value:[w],size:1,done:!1}});const F=E.content;var B=E.size,_=F.toArray(!1),G=_.length;if(G>0){var k=@newArrayWithSize(G);if(@isReadableByteStreamController(K)){{const Q=_[0];if(!(@ArrayBuffer.@isView(Q)||Q instanceof @ArrayBuffer))@putByValDirect(k,0,new @Uint8Array(Q.buffer,Q.byteOffset,Q.byteLength));else @putByValDirect(k,0,Q)}for(var N=1;N<G;N++){const Q=_[N];if(!(@ArrayBuffer.@isView(Q)||Q instanceof @ArrayBuffer))@putByValDirect(k,N,new @Uint8Array(Q.buffer,Q.byteOffset,Q.byteLength));else @putByValDirect(k,N,Q)}}else{@putByValDirect(k,0,_[0].value);for(var N=1;N<G;N++)@putByValDirect(k,N,_[N].value)}if(@resetQueue(@getByIdDirectPrivate(K,\"queue\")),@getByIdDirectPrivate(K,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(K,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(K))@readableStreamDefaultControllerCallPullIfNeeded(K);else if(@isReadableByteStreamController(K))@readableByteStreamControllerCallPullIfNeeded(K);return{value:k,size:B,done:!1}}var U=(Q)=>{if(Q.done)return{value:[],size:0,done:!0};var w=@getByIdDirectPrivate(T,\"readableStreamController\"),d=@getByIdDirectPrivate(w,\"queue\"),H=[Q.value].concat(d.content.toArray(!1)),x=H.length;if(@isReadableByteStreamController(w))for(var I=0;I<x;I++){const A=H[I];if(!(@ArrayBuffer.@isView(A)||A instanceof @ArrayBuffer)){const{buffer:S,byteOffset:D,byteLength:J}=A;@putByValDirect(H,I,new @Uint8Array(S,D,J))}}else for(var I=1;I<x;I++)@putByValDirect(H,I,H[I].value);var j=d.size;if(@resetQueue(d),@getByIdDirectPrivate(w,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(w,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(w))@readableStreamDefaultControllerCallPullIfNeeded(w);else if(@isReadableByteStreamController(w))@readableByteStreamControllerCallPullIfNeeded(w);return{value:H,size:j,done:!1}},W=K.@pull(K);if(W&&@isPromise(W))return W.@then(U);return U(W)})\n";
...
const char* const s_readableStreamPipeThroughCode = "(function (w,O){\"use strict\";const S=w,h=S[\"readable\"];if(!@isReadableStream(h))@throwTypeError(\"readable should be ReadableStream\");const _=S[\"writable\"],j=@getInternalWritableStream(_);if(!@isWritableStream(j))@throwTypeError(\"writable should be WritableStream\");let u=!1,k=!1,q=!1,x;if(!@isUndefinedOrNull(O)){if(!@isObject(O))throw @makeTypeError(\"options must be an object\");if(k=!!O[\"preventAbort\"],q=!!O[\"preventCancel\"],u=!!O[\"preventClose\"],x=O[\"signal\"],x!==@undefined&&!@isAbortSignal(x))throw @makeTypeError(\"options.signal must be AbortSignal\")}if(!@isReadableStream(this))throw @makeThisTypeError(\"ReadableStream\",\"pipeThrough\");if(@isReadableStreamLocked(this))throw @makeTypeError(\"ReadableStream is locked\");if(@isWritableStreamLocked(j))throw @makeTypeError(\"WritableStream is locked\");return @readableStreamPipeToWritableStream(this,j,u,k,q,x),h})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerPullCode = "(function (d){\"use strict\";const P=@getByIdDirectPrivate(d,\"controlledReadableStream\");if(@assert(@readableStreamHasDefaultReader(P)),@getByIdDirectPrivate(d,\"queue\").content\?.isNotEmpty()){const F=@getByIdDirectPrivate(d,\"queue\").content.shift();@getByIdDirectPrivate(d,\"queue\").size-=F.byteLength,@readableByteStreamControllerHandleQueueDrain(d);let R;try{R=new @Uint8Array(F.buffer,F.byteOffset,F.byteLength)}catch(_){return @Promise.@reject(_)}return @createFulfilledPromise({value:R,done:!1})}if(@getByIdDirectPrivate(d,\"autoAllocateChunkSize\")!==@undefined){let F;try{F=@createUninitializedArrayBuffer(@getByIdDirectPrivate(d,\"autoAllocateChunkSize\"))}catch(_){return @Promise.@reject(_)}const R={buffer:F,byteOffset:0,byteLength:@getByIdDirectPrivate(d,\"autoAllocateChunkSize\"),bytesFilled:0,elementSize:1,ctor:@Uint8Array,readerType:\"default\"};@getByIdDirectPrivate(d,\"pendingPullIntos\").push(R)}const h=@readableStreamAddReadRequest(P);return @readableByteStreamControllerCallPullIfNeeded(d),h})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerFillDescriptorFromQueueCode = "(function (_,E){\"use strict\";const q=E.bytesFilled-E.bytesFilled%E.elementSize,j=@getByIdDirectPrivate(_,\"queue\").size<E.byteLength-E.bytesFilled\?@getByIdDirectPrivate(_,\"queue\").size:E.byteLength-E.bytesFilled,w=E.bytesFilled+j,z=w-w%E.elementSize;let k=j,G=!1;if(z>q)k=z-E.bytesFilled,G=!0;while(k>0){let H=@getByIdDirectPrivate(_,\"queue\").content.peek();const v=k<H.byteLength\?k:H.byteLength,J=E.byteOffset+E.bytesFilled;if(new @Uint8Array(E.buffer).set(new @Uint8Array(H.buffer,H.byteOffset,v),J),H.byteLength===v)@getByIdDirectPrivate(_,\"queue\").content.shift();else H.byteOffset+=v,H.byteLength-=v;@getByIdDirectPrivate(_,\"queue\").size-=v,@assert(@getByIdDirectPrivate(_,\"pendingPullIntos\").isEmpty()||@getByIdDirectPrivate(_,\"pendingPullIntos\").peek()===E),@readableByteStreamControllerInvalidateBYOBRequest(_),E.bytesFilled+=v,k-=v}if(!G)@assert(@getByIdDirectPrivate(_,\"queue\").size===0),@assert(E.bytesFilled>0),@assert(E.bytesFilled<E.elementSize);return G})\n";
...
const char* const s_eventSourceGetEventSourceCode = "(function (){\"use strict\";class F extends EventTarget{#$;#j;#w;#A;#B;#F=!1;#G=null;#J=\"\";#K=\"\";#L=\"\";#M=!0;#O=0;#Q=0;#U=0;#V=null;static#W(L){L.#H()}static#X(L,j){const A=L.data,M=A.#L\?`Last-Event-ID: ${A.#L}\\r\\n`:\"\",J=`GET ${j.pathname}${j.search} HTTP/1.1\\r\\nHost: bun\\r\\nContent-type: text/event-stream\\r\\nContent-length: 0\\r\\n${M}\\r\\n`,O=L.write(J);if(O!==J.length)A.#K=J.substring(O)}static#Y(L,j,A){for(;;){if(A>=j.length)return;let M=-1,J=j.indexOf(\"\\r\\n\",A);const O=J+2;if(J>0)if(L.#O===0){const X=parseInt(j.substring(A,J),16);if(X===0){L.#j=2,L.#G\?.end();return}M=O+X}else M=j.length;else{if(L.#J.length===0){L.#J+=j.substring(A);return}M=j.length}let Q=j.substring(O,M);A=M+2;let U=0,V=Q.indexOf(\"\\n\\n\");if(V==-1){L.#J+=j.substring(O);return}if(L.#J.length)L.#J+=Q,Q=L.#J,L.#J=\"\";let W=!0;while(W){const X=Q.substring(U,V);let G,w=\"\",B,Y=0,K=-1;for(;;){let z=X.indexOf(\"\\n\",Y);if(z===-1){if(Y>=X.length)break;z=X.length}const H=X.substring(Y,z);if(H.startsWith(\"data:\"))if(w.length)w+=`\\n${H.substring(5).trim()}`;else w=H.substring(5).trim();else if(H.startsWith(\"event:\"))G=H.substring(6).trim();else if(H.startsWith(\"id:\"))B=H.substring(3).trim();else if(H.startsWith(\"retry:\")){if(K=parseInt(H.substring(6).trim(),10),@isNaN(K))K=-1}Y=z+1}if(L.#L=B||\"\",K>=0)L.#U=K;if(w||B||G)L.dispatchEvent(new MessageEvent(G||\"message\",{data:w||\"\",origin:L.#$.origin,source:L,lastEventId:B}));if(Q.length===V+2){W=!1;break}const Z=Q.indexOf(\"\\n\\n\",V+1);if(Z===-1)break;U=V,V=Z}}}static#Z={open(L){const j=L.data;if(j.#G=L,!j.#F)F.#X(L,j.#$)},handshake(L,j,A){const M=L.data;if(j)F.#X(L,M.#$);else M.#j=2,M.dispatchEvent(new ErrorEvent(\"error\",{error:A})),L.end()},data(L,j){const A=L.data;switch(A.#j){case 0:{let M=j.toString();const J=M.indexOf(\"\\r\\n\\r\\n\");if(J===-1){A.#J+=M;return}if(A.#J.length)A.#J+=M,M=A.#J,A.#J=\"\";const O=M.substring(0,J),Q=O.indexOf(\"\\r\\n\");if(Q===-1){A.#j=2,A.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Invalid HTTP request\")})),L.end();return}const U=O.substring(0,Q);if(U!==\"HTTP/1.1 200 OK\"){A.#j=2,A.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(U)})),L.end();return}let V=Q+1,W=!1,X=-1;for(;;){let w=O.indexOf(\"\\r\\n\",V);if(w===-1){if(V>=O.length){if(!W)A.#j=2,A.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has no MIME type and \"text/event-stream\" is required. Aborting the connection.`)})),L.end();return}w=O.length}const B=O.substring(V+1,w),Y=B.indexOf(\":\"),K=B.substring(0,Y),Z=K.localeCompare(\"content-type\",@undefined,{sensitivity:\"accent\"})===0;if(V=w+1,Z)if(B.endsWith(\" text/event-stream\"))W=!0;else{A.#j=2,A.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has a MIME type that is not \"text/event-stream\". Aborting the connection.`)})),L.end();return}else if(K.localeCompare(\"content-length\",@undefined,{sensitivity:\"accent\"})===0){if(X=parseInt(B.substring(Y+1).trim(),10),@isNaN(X)||X<=0){A.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Content-Length is invalid. Aborting the connection.`)})),L.end();return}if(W)break}else if(K.localeCompare(\"transfer-encoding\",@undefined,{sensitivity:\"accent\"})===0){if(B.substring(Y+1).trim()!==\"chunked\"){A.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Transfer-Encoding is invalid. Aborting the connection.`)})),L.end();return}if(X=0,W)break}}A.#O=X,A.#j=1,A.dispatchEvent(new Event(\"open\"));const G=M.substring(J+4);if(F.#Y(A,G,0),A.#O>0){if(A.#Q+=G.length,A.#Q>=A.#O)A.#j=2,L.end()}return}case 1:if(F.#Y(A,j.toString(),2),A.#O>0){if(A.#Q+=j.byteLength,A.#Q>=A.#O)A.#j=2,L.end()}return;default:break}},drain(L){const j=L.data;if(j.#j===0){const A=j.#J;if(A.length){const M=L.write(A);if(M!==A.length)L.data.#K=A.substring(M);else L.data.#K=\"\"}}},close:F.#z,end(L){F.#z(L).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Connection closed by server\")}))},timeout(L){F.#z(L).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Timeout\")}))},binaryType:\"buffer\"};static#z(L){const j=L.data;if(j.#G=null,j.#Q=0,j.#j=2,j.#M){if(j.#V)clearTimeout(j.#V);j.#V=setTimeout(F.#W,j.#U,j)}return j}constructor(L,j=@undefined){super();const A=new URL(L);this.#F=A.protocol===\"https:\",this.#$=A,this.#j=2,process.nextTick(F.#W,this)}ref(){this.#V\?.ref(),this.#G\?.ref()}unref(){this.#V\?.unref(),this.#G\?.unref()}#H(){if(this.#j!==2)return;const L=this.#$,j=this.#F;this.#j=0,@Bun.connect({data:this,socket:F.#Z,hostname:L.hostname,port:parseInt(L.port||(j\?\"443\":\"80\"),10),tls:j\?{requestCert:!0,rejectUnauthorized:!1}:!1}).catch((A)=>{if(super.dispatchEvent(new ErrorEvent(\"error\",{error:A})),this.#M){if(this.#V)this.#V.unref\?.();this.#V=setTimeout(F.#W,1000,this)}})}get url(){return this.#$.href}get readyState(){return this.#j}close(){this.#M=!1,this.#j=2,this.#G\?.unref(),this.#G\?.end()}get onopen(){return this.#B}get onerror(){return this.#w}get onmessage(){return this.#A}set onopen(L){if(this.#B)super.removeEventListener(\"close\",this.#B);super.addEventListener(\"open\",L),this.#B=L}set onerror(L){if(this.#w)super.removeEventListener(\"error\",this.#w);super.addEventListener(\"error\",L),this.#w=L}set onmessage(L){if(this.#A)super.removeEventListener(\"message\",this.#A);super.addEventListener(\"message\",L),this.#A=L}}return Object.defineProperty(F.prototype,\"CONNECTING\",{enumerable:!0,value:0}),Object.defineProperty(F.prototype,\"OPEN\",{enumerable:!0,value:1}),Object.defineProperty(F.prototype,\"CLOSED\",{enumerable:!0,value:2}),F[Symbol.for(\"CommonJS\")]=0,F})\n";
