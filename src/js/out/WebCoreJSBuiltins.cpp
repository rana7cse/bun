...
const char* const s_transformStreamInternalsCreateTransformStreamCode = "(function (j,v,x,B,c,E,q){\"use strict\";if(B===@undefined)B=1;if(c===@undefined)c=()=>1;if(E===@undefined)E=0;if(q===@undefined)q=()=>1;@assert(B>=0),@assert(E>=0);const F={};@putByIdDirectPrivate(F,\"TransformStream\",!0);const G=new @TransformStream(F),I=@newPromiseCapability(@Promise);@initializeTransformStream(G,I.@promise,B,c,E,q);const D=new @TransformStreamDefaultController;return @setUpTransformStreamDefaultController(G,D,v,x),j().@then(()=>{I.@resolve.@call()},(_)=>{I.@reject.@call(@undefined,_)}),G})\n";
...
const char* const s_transformStreamInternalsInitializeTransformStreamCode = "(function (q,v,x,E,f,I){\"use strict\";const T=()=>{return v},J=(j)=>{return @transformStreamDefaultSinkWriteAlgorithm(q,j)},K=(j)=>{return @transformStreamDefaultSinkAbortAlgorithm(q,j)},L=()=>{return @transformStreamDefaultSinkCloseAlgorithm(q)},F=@createWritableStream(T,J,L,K,x,E),B=()=>{return @transformStreamDefaultSourcePullAlgorithm(q)},C=(j)=>{return @transformStreamErrorWritableAndUnblockWrite(q,j),@Promise.@resolve()},G={};@putByIdDirectPrivate(G,\"start\",T),@putByIdDirectPrivate(G,\"pull\",B),@putByIdDirectPrivate(G,\"cancel\",C);const D={};@putByIdDirectPrivate(D,\"size\",I),@putByIdDirectPrivate(D,\"highWaterMark\",f);const N=new @ReadableStream(G,D);@putByIdDirectPrivate(q,\"writable\",F),@putByIdDirectPrivate(q,\"internalWritable\",@getInternalWritableStream(F)),@putByIdDirectPrivate(q,\"readable\",N),@putByIdDirectPrivate(q,\"backpressure\",@undefined),@putByIdDirectPrivate(q,\"backpressureChangePromise\",@undefined),@transformStreamSetBackpressure(q,!0),@putByIdDirectPrivate(q,\"controller\",@undefined)})\n";
...
const char* const s_transformStreamInitializeTransformStreamCode = "(function (){\"use strict\";let j=arguments[0];if(@isObject(j)&&@getByIdDirectPrivate(j,\"TransformStream\"))return this;let v=arguments[1],q=arguments[2];if(j===@undefined)j=null;if(q===@undefined)q={};if(v===@undefined)v={};let x={};if(j!==null){if(\"start\"in j){if(x[\"start\"]=j[\"start\"],typeof x[\"start\"]!==\"function\")@throwTypeError(\"transformer.start should be a function\")}if(\"transform\"in j){if(x[\"transform\"]=j[\"transform\"],typeof x[\"transform\"]!==\"function\")@throwTypeError(\"transformer.transform should be a function\")}if(\"flush\"in j){if(x[\"flush\"]=j[\"flush\"],typeof x[\"flush\"]!==\"function\")@throwTypeError(\"transformer.flush should be a function\")}if(\"readableType\"in j)@throwRangeError(\"TransformStream transformer has a readableType\");if(\"writableType\"in j)@throwRangeError(\"TransformStream transformer has a writableType\")}const B=@extractHighWaterMark(q,0),E=@extractSizeAlgorithm(q),F=@extractHighWaterMark(v,1),G=@extractSizeAlgorithm(v),I=@newPromiseCapability(@Promise);if(@initializeTransformStream(this,I.@promise,F,G,B,E),@setUpTransformStreamDefaultControllerFromTransformer(this,j,x),(\"start\"in x)){const J=@getByIdDirectPrivate(this,\"controller\");(()=>@promiseInvokeOrNoopMethodNoCatch(j,x[\"start\"],[J]))().@then(()=>{I.@resolve.@call()},(u)=>{I.@reject.@call(@undefined,u)})}else I.@resolve.@call();return this})\n";
...
const char* const s_jsBufferPrototypeSliceCode = "(function (c,k){\"use strict\";var{buffer:p,byteOffset:m,byteLength:q}=this;function v(x,z){if(x=@trunc(x),x===0||@isNaN(x))return 0;else if(x<0)return x+=z,x>0\?x:0;else return x<z\?x:z}var i=v(c,q),w=k!==@undefined\?v(k,q):q;return new @Buffer(p,m+i,w>i\?w-i:0)})\n";
...
const char* const s_consoleObjectWriteCode = "(function (d){\"use strict\";var b=@getByIdDirectPrivate(this,\"writer\");if(!b){var c=@toLength(d\?.length\?\?0);b=@Bun.stdout.writer({highWaterMark:c>65536\?c:65536}),@putByIdDirectPrivate(this,\"writer\",b)}var a=b.write(d);const _=@argumentCount();for(var f=1;f<_;f++)a+=b.write(@argument(f));return b.flush(!0),a})\n";
...
const char* const s_readableStreamInternalsSetupReadableStreamDefaultControllerCode = "(function (f,_,q,w,v,b,j){\"use strict\";const x=new @ReadableStreamDefaultController(f,_,q,w,@isReadableStream),B=()=>@promiseInvokeOrNoopMethod(_,b,[x]),C=(D)=>@promiseInvokeOrNoopMethod(_,j,[D]);@putByIdDirectPrivate(x,\"pullAlgorithm\",B),@putByIdDirectPrivate(x,\"cancelAlgorithm\",C),@putByIdDirectPrivate(x,\"pull\",@readableStreamDefaultControllerPull),@putByIdDirectPrivate(x,\"cancel\",@readableStreamDefaultControllerCancel),@putByIdDirectPrivate(f,\"readableStreamController\",x),@readableStreamDefaultControllerStart(x)})\n";
...
const char* const s_readableStreamInternalsReadableStreamCancelCode = "(function (_,d){\"use strict\";@putByIdDirectPrivate(_,\"disturbed\",!0);const j=@getByIdDirectPrivate(_,\"state\");if(j===@streamClosed)return @Promise.@resolve();if(j===@streamErrored)return @Promise.@reject(@getByIdDirectPrivate(_,\"storedError\"));@readableStreamClose(_);var h=@getByIdDirectPrivate(_,\"readableStreamController\"),p=h.@cancel;if(p)return p(h,d).@then(function(){});var i=h.close;if(i)return @Promise.@resolve(h.close(d));return @Promise.@reject(@makeTypeError(\"ReadableStreamController has no cancel or close method\"))})\n";
...
const char* const s_readableStreamInternalsReadableStreamDefaultControllerPullCode = "(function (d){\"use strict\";var a=@getByIdDirectPrivate(d,\"queue\");if(a.content.isNotEmpty()){const f=@dequeueValue(a);if(@getByIdDirectPrivate(d,\"closeRequested\")&&a.content.isEmpty())@readableStreamClose(@getByIdDirectPrivate(d,\"controlledReadableStream\"));else @readableStreamDefaultControllerCallPullIfNeeded(d);return @createFulfilledPromise({value:f,done:!1})}const _=@readableStreamAddReadRequest(@getByIdDirectPrivate(d,\"controlledReadableStream\"));return @readableStreamDefaultControllerCallPullIfNeeded(d),_})\n";
...
const char* const s_readableStreamInternalsReadableStreamToArrayDirectCode = "(async function (_,k){\"use strict\";const q=@initializeArrayStream.@call(_,k,@undefined);k=@undefined;var f=_.getReader();try{while(@getByIdDirectPrivate(_,\"state\")===@streamReadable){var j=await f.read();if(j.done)break}try{f.releaseLock()}catch(p){}return f=@undefined,@Promise.@resolve(q.@promise)}catch(p){throw p}finally{_=@undefined,f=@undefined}})\n";
...
const char* const s_eventStreamGetEventStreamCode = "(function (){\"use strict\";class _ extends @ReadableStream{#_;@contentType=\"text/event-stream\";constructor(j){if(!j||!@isCallable(j.start))@throwTypeError(\"EventStream requires an object with `start`\");var g=[];super({type:\"direct\",pull:(y)=>{if(this.#_=y,g.length){for(let k of g)if(y.write(k),k===null){y.close(),this.#_=null;return}y.flush()}j.start\?.(this)},cancel:()=>{j.cancel\?.(this),this.#_=null}});this.#_={write:(y)=>g.push(y),flush:()=>{},close:()=>{g.push(null)}}}setReconnectionTime(j){var g=this.#_;if(!g)throw new Error(\"EventStream is closed\");g.write(\"retry:\"+j+\"\\n\\n\")}send(j,g,y){var k=this.#_;if(!k)throw new Error(\"EventStream is closed\");if(!g)g=j,j=@undefined;else if(j===\"message\")j=@undefined;if(g===@undefined)@throwTypeError(\"EventStream.send() requires a data argument\");if(y!==@undefined)this.#j(k,j,g,y);else this.#$(k,j,g);k.flush()}#$(j,g,y){if(g)j.write(\"event:\"+g+\"\\n\");if(typeof y===\"string\")j.write(\"data:\"+y.replace(/\\n/g,\"\\ndata:\")+\"\\n\\n\");else if(@isTypedArrayView(y)||y instanceof @ArrayBuffer)j.write(\"data:\"),j.write(y),j.write(\"\\n\\n\");else j.write(\"data:\"+JSON.stringify(y)+\"\\n\\n\")}#j(j,g,y,k){if(g)j.write(\"event:\"+g+\"\\n\");if(typeof y===\"string\")j.write(\"data:\"+y.replace(/\\n/g,\"\\ndata:\")+\"\\n\");else if(y instanceof @Uint8Array)j.write(\"data:\"),j.write(y),j.write(\"\\n\");else j.write(\"data:\"+JSON.stringify(y)+\"\\n\");if(k===null)j.write(\"id\\n\");else j.write(\"id:\"+k+\"\\n\")}close(){this.#_\?.close()}}return Object.defineProperty(_,\"name\",{value:\"EventStream\"}),_})\n";
...
const char* const s_readableStreamDefaultReaderReadManyCode = "(function (){\"use strict\";if(!@isReadableStreamDefaultReader(this))@throwTypeError(\"ReadableStreamDefaultReader.readMany() should not be called directly\");const N=@getByIdDirectPrivate(this,\"ownerReadableStream\");if(!N)@throwTypeError(\"readMany() called on a reader owned by no readable stream\");const d=@getByIdDirectPrivate(N,\"state\");if(@putByIdDirectPrivate(N,\"disturbed\",!0),d===@streamClosed)return{value:[],size:0,done:!0};else if(d===@streamErrored)throw @getByIdDirectPrivate(N,\"storedError\");var B=@getByIdDirectPrivate(N,\"readableStreamController\"),w=@getByIdDirectPrivate(B,\"queue\");if(!w)return B.@pull(B).@then(function({done:S,value:T}){return S\?{done:!0,value:[],size:0}:{value:[T],size:1,done:!1}});const A=w.content;var k=w.size,J=A.toArray(!1),F=J.length;if(F>0){var _=@newArrayWithSize(F);if(@isReadableByteStreamController(B)){{const S=J[0];if(!(@ArrayBuffer.@isView(S)||S instanceof @ArrayBuffer))@putByValDirect(_,0,new @Uint8Array(S.buffer,S.byteOffset,S.byteLength));else @putByValDirect(_,0,S)}for(var D=1;D<F;D++){const S=J[D];if(!(@ArrayBuffer.@isView(S)||S instanceof @ArrayBuffer))@putByValDirect(_,D,new @Uint8Array(S.buffer,S.byteOffset,S.byteLength));else @putByValDirect(_,D,S)}}else{@putByValDirect(_,0,J[0].value);for(var D=1;D<F;D++)@putByValDirect(_,D,J[D].value)}if(@resetQueue(@getByIdDirectPrivate(B,\"queue\")),@getByIdDirectPrivate(B,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(B,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(B))@readableStreamDefaultControllerCallPullIfNeeded(B);else if(@isReadableByteStreamController(B))@readableByteStreamControllerCallPullIfNeeded(B);return{value:_,size:k,done:!1}}var K=(S)=>{if(S.done)return{value:[],size:0,done:!0};var T=@getByIdDirectPrivate(N,\"readableStreamController\"),U=@getByIdDirectPrivate(T,\"queue\"),W=[S.value].concat(U.content.toArray(!1)),j=W.length;if(@isReadableByteStreamController(T))for(var C=0;C<j;C++){const x=W[C];if(!(@ArrayBuffer.@isView(x)||x instanceof @ArrayBuffer)){const{buffer:I,byteOffset:H,byteLength:E}=x;@putByValDirect(W,C,new @Uint8Array(I,H,E))}}else for(var C=1;C<j;C++)@putByValDirect(W,C,W[C].value);var G=U.size;if(@resetQueue(U),@getByIdDirectPrivate(T,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(T,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(T))@readableStreamDefaultControllerCallPullIfNeeded(T);else if(@isReadableByteStreamController(T))@readableByteStreamControllerCallPullIfNeeded(T);return{value:W,size:G,done:!1}},Q=B.@pull(B);if(Q&&@isPromise(Q))return Q.@then(K);return K(Q)})\n";
...
const char* const s_streamInternalsPromiseInvokeOrNoopNoCatchCode = "(function (l,d,n){\"use strict\";return @promiseInvokeOrNoopMethodNoCatch(l,l[d],n)})\n";
...
const char* const s_importMetaObjectInternalRequireCode = "(function (_){\"use strict\";var b=@requireMap.@get(_);const j=_.substring(_.length-5);if(b)return b.exports;if(j===\".json\"){var k=globalThis[Symbol.for(\"_fs\")]||=@Bun.fs(),q=JSON.parse(k.readFileSync(_,\"utf8\"));return @requireMap.@set(_,@createCommonJSModule(_,q,!0)),q}else if(j===\".node\"){const g=@createCommonJSModule(_,{},!0);return process.dlopen(g,_),@requireMap.@set(_,g),g.exports}else if(j===\".toml\"){var k=globalThis[Symbol.for(\"_fs\")]||=@Bun.fs(),q=@Bun.TOML.parse(k.readFileSync(_,\"utf8\"));return @requireMap.@set(_,@createCommonJSModule(_,q,!0)),q}else{var q=@requireESM(_);const n=@requireMap.@get(_);if(n)return n.exports;var v=q\?.default;if(v\?.[@commonJSSymbol]===0)q=v;return @requireMap.@set(_,@createCommonJSModule(_,q,!0)),q}})\n";
...
const char* const s_readableStreamPipeThroughCode = "(function (u,_){\"use strict\";const y=u,S=y[\"readable\"];if(!@isReadableStream(S))@throwTypeError(\"readable should be ReadableStream\");const O=y[\"writable\"],d=@getInternalWritableStream(O);if(!@isWritableStream(d))@throwTypeError(\"writable should be WritableStream\");let c=!1,h=!1,j=!1,k;if(!@isUndefinedOrNull(_)){if(!@isObject(_))@throwTypeError(\"options must be an object\");if(h=!!_[\"preventAbort\"],j=!!_[\"preventCancel\"],c=!!_[\"preventClose\"],k=_[\"signal\"],k!==@undefined&&!@isAbortSignal(k))@throwTypeError(\"options.signal must be AbortSignal\")}if(!@isReadableStream(this))throw @makeThisTypeError(\"ReadableStream\",\"pipeThrough\");if(@isReadableStreamLocked(this))@throwTypeError(\"ReadableStream is locked\");if(@isWritableStreamLocked(d))@throwTypeError(\"WritableStream is locked\");return @readableStreamPipeToWritableStream(this,d,c,h,j,k),S})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerPullCode = "(function (d){\"use strict\";const _=@getByIdDirectPrivate(d,\"controlledReadableStream\");if(@assert(@readableStreamHasDefaultReader(_)),@getByIdDirectPrivate(d,\"queue\").content\?.isNotEmpty()){const P=@getByIdDirectPrivate(d,\"queue\").content.shift();@getByIdDirectPrivate(d,\"queue\").size-=P.byteLength,@readableByteStreamControllerHandleQueueDrain(d);let F;try{F=new @Uint8Array(P.buffer,P.byteOffset,P.byteLength)}catch(R){return @Promise.@reject(R)}return @createFulfilledPromise({value:F,done:!1})}if(@getByIdDirectPrivate(d,\"autoAllocateChunkSize\")!==@undefined){let P;try{P=@createUninitializedArrayBuffer(@getByIdDirectPrivate(d,\"autoAllocateChunkSize\"))}catch(R){return @Promise.@reject(R)}const F={buffer:P,byteOffset:0,byteLength:@getByIdDirectPrivate(d,\"autoAllocateChunkSize\"),bytesFilled:0,elementSize:1,ctor:@Uint8Array,readerType:\"default\"};@getByIdDirectPrivate(d,\"pendingPullIntos\").push(F)}const h=@readableStreamAddReadRequest(_);return @readableByteStreamControllerCallPullIfNeeded(d),h})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerFillDescriptorFromQueueCode = "(function (q,k){\"use strict\";const v=k.bytesFilled-k.bytesFilled%k.elementSize,w=@getByIdDirectPrivate(q,\"queue\").size<k.byteLength-k.bytesFilled\?@getByIdDirectPrivate(q,\"queue\").size:k.byteLength-k.bytesFilled,_=k.bytesFilled+w,E=_-_%k.elementSize;let j=w,G=!1;if(E>v)j=E-k.bytesFilled,G=!0;while(j>0){let H=@getByIdDirectPrivate(q,\"queue\").content.peek();const J=j<H.byteLength\?j:H.byteLength,z=k.byteOffset+k.bytesFilled;if(new @Uint8Array(k.buffer).set(new @Uint8Array(H.buffer,H.byteOffset,J),z),H.byteLength===J)@getByIdDirectPrivate(q,\"queue\").content.shift();else H.byteOffset+=J,H.byteLength-=J;@getByIdDirectPrivate(q,\"queue\").size-=J,@assert(@getByIdDirectPrivate(q,\"pendingPullIntos\").isEmpty()||@getByIdDirectPrivate(q,\"pendingPullIntos\").peek()===k),@readableByteStreamControllerInvalidateBYOBRequest(q),k.bytesFilled+=J,j-=J}if(!G)@assert(@getByIdDirectPrivate(q,\"queue\").size===0),@assert(k.bytesFilled>0),@assert(k.bytesFilled<k.elementSize);return G})\n";
