...
const char* const s_writableStreamInternalsWritableStreamFinishErroringCode = "(function (i){\"use strict\";@assert(@getByIdDirectPrivate(i,\"state\")===\"erroring\"),@assert(!@writableStreamHasOperationMarkedInFlight(i)),@putByIdDirectPrivate(i,\"state\",\"errored\");const p=@getByIdDirectPrivate(i,\"controller\");@getByIdDirectPrivate(p,\"errorSteps\").@call();const A=@getByIdDirectPrivate(i,\"storedError\"),B=@getByIdDirectPrivate(i,\"writeRequests\");for(var _=B.shift();_;_=B.shift())_.@reject.@call(@undefined,A);@putByIdDirectPrivate(i,\"writeRequests\",@createFIFO());const h=@getByIdDirectPrivate(i,\"pendingAbortRequest\");if(h===@undefined){@writableStreamRejectCloseAndClosedPromiseIfNeeded(i);return}if(@putByIdDirectPrivate(i,\"pendingAbortRequest\",@undefined),h.wasAlreadyErroring){h.promise.@reject.@call(@undefined,A),@writableStreamRejectCloseAndClosedPromiseIfNeeded(i);return}@getByIdDirectPrivate(p,\"abortSteps\").@call(@undefined,h.reason).@then(()=>{h.promise.@resolve.@call(),@writableStreamRejectCloseAndClosedPromiseIfNeeded(i)},(I)=>{h.promise.@reject.@call(@undefined,I),@writableStreamRejectCloseAndClosedPromiseIfNeeded(i)})})\n";
...
const char* const s_writableStreamInternalsWritableStreamDefaultWriterWriteCode = "(function (d,W){\"use strict\";const _=@getByIdDirectPrivate(d,\"stream\");@assert(_!==@undefined);const P=@getByIdDirectPrivate(_,\"controller\");@assert(P!==@undefined);const b=@writableStreamDefaultControllerGetChunkSize(P,W);if(_!==@getByIdDirectPrivate(d,\"stream\"))return @Promise.@reject(@makeTypeError(\"writer is not stream's writer\"));const f=@getByIdDirectPrivate(_,\"state\");if(f===\"errored\")return @Promise.@reject(@getByIdDirectPrivate(_,\"storedError\"));if(@writableStreamCloseQueuedOrInFlight(_)||f===\"closed\")return @Promise.@reject(@makeTypeError(\"stream is closing or closed\"));if(@writableStreamCloseQueuedOrInFlight(_)||f===\"closed\")return @Promise.@reject(@makeTypeError(\"stream is closing or closed\"));if(f===\"erroring\")return @Promise.@reject(@getByIdDirectPrivate(_,\"storedError\"));@assert(f===\"writable\");const g=@writableStreamAddWriteRequest(_);return @writableStreamDefaultControllerWrite(P,W,b),g})\n";
...
const char* const s_transformStreamInternalsInitializeTransformStreamCode = "(function (j,D,q,I,J,K){\"use strict\";const f=()=>{return D},L=(G)=>{return @transformStreamDefaultSinkWriteAlgorithm(j,G)},N=(G)=>{return @transformStreamDefaultSinkAbortAlgorithm(j,G)},v=()=>{return @transformStreamDefaultSinkCloseAlgorithm(j)},E=@createWritableStream(f,L,v,N,q,I),T=()=>{return @transformStreamDefaultSourcePullAlgorithm(j)},F=(G)=>{return @transformStreamErrorWritableAndUnblockWrite(j,G),@Promise.@resolve()},B={};@putByIdDirectPrivate(B,\"start\",f),@putByIdDirectPrivate(B,\"pull\",T),@putByIdDirectPrivate(B,\"cancel\",F);const C={};@putByIdDirectPrivate(C,\"size\",K),@putByIdDirectPrivate(C,\"highWaterMark\",J);const x=new @ReadableStream(B,C);@putByIdDirectPrivate(j,\"writable\",E),@putByIdDirectPrivate(j,\"internalWritable\",@getInternalWritableStream(E)),@putByIdDirectPrivate(j,\"readable\",x),@putByIdDirectPrivate(j,\"backpressure\",@undefined),@putByIdDirectPrivate(j,\"backpressureChangePromise\",@undefined),@transformStreamSetBackpressure(j,!0),@putByIdDirectPrivate(j,\"controller\",@undefined)})\n";
...
const char* const s_transformStreamInternalsTransformStreamErrorCode = "(function (_,f){\"use strict\";const n=@getByIdDirectPrivate(_,\"readable\"),i=@getByIdDirectPrivate(n,\"readableStreamController\");@readableStreamDefaultControllerError(i,f),@transformStreamErrorWritableAndUnblockWrite(_,f)})\n";
...
const char* const s_transformStreamInternalsSetUpTransformStreamDefaultControllerCode = "(function (b,d,_,P){\"use strict\";@assert(@isTransformStream(b)),@assert(@getByIdDirectPrivate(b,\"controller\")===@undefined),@putByIdDirectPrivate(d,\"stream\",b),@putByIdDirectPrivate(b,\"controller\",d),@putByIdDirectPrivate(d,\"transformAlgorithm\",_),@putByIdDirectPrivate(d,\"flushAlgorithm\",P)})\n";
...
const char* const s_transformStreamInternalsTransformStreamDefaultControllerTerminateCode = "(function (i){\"use strict\";const k=@getByIdDirectPrivate(i,\"stream\"),f=@getByIdDirectPrivate(k,\"readable\"),u=@getByIdDirectPrivate(f,\"readableStreamController\");if(@readableStreamDefaultControllerCanCloseOrEnqueue(u))@readableStreamDefaultControllerClose(u);const _=@makeTypeError(\"the stream has been terminated\");@transformStreamErrorWritableAndUnblockWrite(k,_)})\n";
...
const char* const s_transformStreamInternalsTransformStreamDefaultSinkWriteAlgorithmCode = "(function (_,d){\"use strict\";const v=@getByIdDirectPrivate(_,\"internalWritable\");@assert(@getByIdDirectPrivate(v,\"state\")===\"writable\");const j=@getByIdDirectPrivate(_,\"controller\");if(@getByIdDirectPrivate(_,\"backpressure\")){const q=@newPromiseCapability(@Promise),f=@getByIdDirectPrivate(_,\"backpressureChangePromise\");return @assert(f!==@undefined),f.@promise.@then(()=>{const x=@getByIdDirectPrivate(v,\"state\");if(x===\"erroring\"){q.@reject.@call(@undefined,@getByIdDirectPrivate(v,\"storedError\"));return}@assert(x===\"writable\"),@transformStreamDefaultControllerPerformTransform(j,d).@then(()=>{q.@resolve()},(z)=>{q.@reject.@call(@undefined,z)})},(x)=>{q.@reject.@call(@undefined,x)}),q.@promise}return @transformStreamDefaultControllerPerformTransform(j,d)})\n";
...
const char* const s_processObjectInternalsBindingCode = "(function (l){\"use strict\";if(l!==\"constants\")@throwTypeError(\"process.binding() is not supported in Bun. If that breaks something, please file an issue and include a reproducible code sample.\");var r=globalThis.Symbol.for(\"process.bindings.constants\"),d=globalThis[r];if(!d){const{constants:I}=globalThis[globalThis.Symbol.for(\"Bun.lazy\")](\"createImportMeta\",\"node:process\").require(\"node:fs\");d={fs:I,zlib:{},crypto:{},os:@Bun._Os().constants},globalThis[r]=d}return d})\n";
...
const char* const s_transformStreamInitializeTransformStreamCode = "(function (){\"use strict\";let _=arguments[0];if(@isObject(_)&&@getByIdDirectPrivate(_,\"TransformStream\"))return this;let v=arguments[1],x=arguments[2];if(_===@undefined)_=null;if(x===@undefined)x={};if(v===@undefined)v={};let B={};if(_!==null){if(\"start\"in _){if(B[\"start\"]=_[\"start\"],typeof B[\"start\"]!==\"function\")@throwTypeError(\"transformer.start should be a function\")}if(\"transform\"in _){if(B[\"transform\"]=_[\"transform\"],typeof B[\"transform\"]!==\"function\")@throwTypeError(\"transformer.transform should be a function\")}if(\"flush\"in _){if(B[\"flush\"]=_[\"flush\"],typeof B[\"flush\"]!==\"function\")@throwTypeError(\"transformer.flush should be a function\")}if(\"readableType\"in _)@throwRangeError(\"TransformStream transformer has a readableType\");if(\"writableType\"in _)@throwRangeError(\"TransformStream transformer has a writableType\")}const E=@extractHighWaterMark(x,0),F=@extractSizeAlgorithm(x),j=@extractHighWaterMark(v,1),q=@extractSizeAlgorithm(v),G=@newPromiseCapability(@Promise);if(@initializeTransformStream(this,G.@promise,j,q,E,F),@setUpTransformStreamDefaultControllerFromTransformer(this,_,B),(\"start\"in B)){const u=@getByIdDirectPrivate(this,\"controller\");(()=>@promiseInvokeOrNoopMethodNoCatch(_,B[\"start\"],[u]))().@then(()=>{G.@resolve.@call()},(J)=>{G.@reject.@call(@undefined,J)})}else G.@resolve.@call();return this})\n";
...
const char* const s_moduleRequireCode = "(function (_){\"use strict\";const M=@requireMap.@get(_)||@requireMap.@get(_=@resolveSync(_,this.path,!1));if(M)return @evaluateCommonJSModule(M),M.exports;if(_.endsWith(\".json\")||_.endsWith(\".toml\")||_.endsWith(\".node\"))return @internalRequire(_);let S=@Loader.registry.@get(_);if(S\?.evaluated&&(S.state\?\?0)>=@ModuleReady){const h=S.module,r=@Loader.getModuleNamespaceObject(h),f=r\?.[@commonJSSymbol]===0||r\?.default\?.[@commonJSSymbol]===0\?r.default:r.__esModule\?r:Object.create(r,{__esModule:{value:!0}});return @requireMap.@set(_,@createCommonJSModule(_,f,!0)),f}const L=@createCommonJSModule(_,{},!1);@requireMap.@set(_,L);var b=this.@require(_,L);if(b===-1){try{b=@requireESM(_)}catch(h){throw @requireMap.@delete(_),h}if(S=@Loader.registry.@get(_),S\?.evaluated&&(S.state\?\?0)>=@ModuleReady){const h=@Loader.getModuleNamespaceObject(S.module);return L.exports=h\?.[@commonJSSymbol]===0||h\?.default\?.[@commonJSSymbol]===0\?h.default:h.__esModule\?h:Object.create(h,{__esModule:{value:!0}})}}return @evaluateCommonJSModule(L),L.exports})\n";
...
const char* const s_jsBufferPrototypeReadUIntBECode = "(function (r,p){\"use strict\";const d=this.@dataView||=new DataView(this.buffer,this.byteOffset,this.byteLength);switch(p){case 1:return d.getUint8(r);case 2:return d.getUint16(r,!1);case 3:return d.getUint16(r+1,!1)+d.getUint8(r)*65536;case 4:return d.getUint32(r,!1);case 5:{const c=d.getUint8(r);return(c|(c&128)*33554430)*4294967296+d.getUint32(r+1,!1)}case 6:{const c=d.getUint16(r,!1);return(c|(c&32768)*131070)*4294967296+d.getUint32(r+2,!1)}}@throwRangeError(\"byteLength must be >= 1 and <= 6\")})\n";
...
const char* const s_consoleObjectAsyncIteratorCode = "(function (){\"use strict\";const _=async function*A(){var m=@Bun.stdin.stream().getReader(),D=new globalThis.TextDecoder(\"utf-8\",{fatal:!1}),q,F=@Bun.indexOfLine;try{while(!0){var z,H,G;const L=m.readMany();if(@isPromise(L))({done:z,value:H}=await L);else({done:z,value:H}=L);if(z){if(G)yield D.decode(G);return}var w;for(let M of H){if(w=M,G)w=@Buffer.concat([G,M]),G=null;var J=0,K=F(w,J);while(K!==-1)yield D.decode(w.subarray(J,K)),J=K+1,K=F(w,J);G=w.subarray(J)}}}catch(L){q=L}finally{if(m.releaseLock(),q)throw q}},j=globalThis.Symbol.asyncIterator;return this[j]=_,_()})\n";
...
const char* const s_consoleObjectWriteCode = "(function (a){\"use strict\";var d=@getByIdDirectPrivate(this,\"writer\");if(!d){var b=@toLength(a\?.length\?\?0);d=@Bun.stdout.writer({highWaterMark:b>65536\?b:65536}),@putByIdDirectPrivate(this,\"writer\",d)}var _=d.write(a);const c=@argumentCount();for(var f=1;f<c;f++)_+=d.write(@argument(f));return d.flush(!0),_})\n";
...
const char* const s_readableStreamInternalsAssignToStreamCode = "(function (_,h){\"use strict\";var b=@getByIdDirectPrivate(_,\"underlyingSource\");if(b)try{return @readDirectStream(_,h,b)}catch(f){throw f}finally{b=@undefined,_=@undefined,h=@undefined}return @readStreamIntoSink(_,h,!0)})\n";
...
const char* const s_readableStreamInternalsHandleDirectStreamErrorCode = "(function (u){\"use strict\";var h=this,R=h.@sink;if(R){@putByIdDirectPrivate(h,\"sink\",@undefined);try{R.close(u)}catch(i){}}if(this.error=this.flush=this.write=this.close=this.end=@onReadableStreamDirectControllerClosed,typeof this.@underlyingSource.close===\"function\")try{this.@underlyingSource.close.@call(this.@underlyingSource,u)}catch(i){}try{var _=h._pendingRead;if(_)h._pendingRead=@undefined,@rejectPromise(_,u)}catch(i){}var a=h.@controlledReadableStream;if(a)@readableStreamError(a,u)})\n";
...
const char* const s_readableStreamInternalsLazyLoadStreamCode = "(function (B,f){\"use strict\";var q=@getByIdDirectPrivate(B,\"bunNativeType\"),j=@getByIdDirectPrivate(B,\"bunNativePtr\"),m=@lazyStreamPrototypeMap.@get(q);if(m===@undefined){let Q=function(Y){var{c:Z,v:_}=this;this.c=@undefined,this.v=@undefined,I(Y,Z,_)},U=function(Y){try{Y.close()}catch(Z){globalThis.reportError(Z)}},W=function(Y,Z,_,p){p[0]=!1;var z;try{z=D(Y,_,p)}catch(A){return Z.error(A)}return I(z,Z,_)};var O=Q,P=U,N=W,[D,E,F,b,x,y,G]=@lazyLoad(q),H=[!1],I;I=function Y(Z,_,p){if(Z&&@isPromise(Z))return Z.then(Q.bind({c:_,v:p}),(z)=>_.error(z));else if(typeof Z===\"number\")if(p&&p.byteLength===Z&&p.buffer===_.byobRequest\?.view\?.buffer)_.byobRequest.respondWithNewView(p);else _.byobRequest.respond(Z);else if(Z.constructor===@Uint8Array)_.enqueue(Z);if(H[0]||Z===!1)@enqueueJob(U,_),H[0]=!1};const X=x\?new FinalizationRegistry(x):null;m=class Y{constructor(Z,_,p){if(this.#f=Z,this.#b={},this.pull=this.#j.bind(this),this.cancel=this.#m.bind(this),this.autoAllocateChunkSize=_,p!==@undefined)this.start=(z)=>{z.enqueue(p)};if(X)X.register(this,Z,this.#b)}#b;pull;cancel;start;#f;type=\"bytes\";autoAllocateChunkSize=0;static startSync=E;#j(Z){var _=this.#f;if(!_){Z.close();return}W(_,Z,Z.byobRequest.view,H)}#m(Z){var _=this.#f;X&&X.unregister(this.#b),y&&y(_,!1),F(_,Z)}static deinit=x;static drain=G},@lazyStreamPrototypeMap.@set(q,m)}const J=m.startSync(j,f);var K;const{drain:L,deinit:M}=m;if(L)K=L(j);if(J===0){if(x&&j&&@enqueueJob(x,j),(K\?.byteLength\?\?0)>0)return{start(Q){Q.enqueue(K),Q.close()},type:\"bytes\"};return{start(Q){Q.close()},type:\"bytes\"}}return new m(j,J,K)})\n";
...
const char* const s_readableStreamInternalsReadableStreamToArrayBufferDirectCode = "(function (q,v){\"use strict\";var w=new @Bun.ArrayBufferSink;@putByIdDirectPrivate(q,\"underlyingSource\",@undefined);var x=@getByIdDirectPrivate(q,\"highWaterMark\");w.start(x\?{highWaterMark:x}:{});var O=@newPromiseCapability(@Promise),C=!1,j=v.pull,D=v.close,F={start(){},close(A){if(!C){if(C=!0,D)D();@fulfillPromise(O.@promise,w.end())}},end(){if(!C){if(C=!0,D)D();@fulfillPromise(O.@promise,w.end())}},flush(){return 0},write:w.write.bind(w)},G=!1;try{const A=j(F);if(A&&@isObject(A)&&@isPromise(A))return async function(_,z,B){while(!C)await B(_);return await z}(F,promise,j);return O.@promise}catch(A){return G=!0,@readableStreamError(q,A),@Promise.@reject(A)}finally{if(!G&&q)@readableStreamClose(q);F=D=w=j=q=@undefined}})\n";
...
const char* const s_readableStreamDefaultReaderReadManyCode = "(function (){\"use strict\";if(!@isReadableStreamDefaultReader(this))@throwTypeError(\"ReadableStreamDefaultReader.readMany() should not be called directly\");const K=@getByIdDirectPrivate(this,\"ownerReadableStream\");if(!K)@throwTypeError(\"readMany() called on a reader owned by no readable stream\");const k=@getByIdDirectPrivate(K,\"state\");if(@putByIdDirectPrivate(K,\"disturbed\",!0),k===@streamClosed)return{value:[],size:0,done:!0};else if(k===@streamErrored)throw @getByIdDirectPrivate(K,\"storedError\");var A=@getByIdDirectPrivate(K,\"readableStreamController\"),x=@getByIdDirectPrivate(A,\"queue\");if(!x)return A.@pull(A).@then(function({done:S,value:T}){return S\?{done:!0,value:[],size:0}:{value:[T],size:1,done:!1}});const C=x.content;var _=x.size,H=C.toArray(!1),N=H.length;if(N>0){var D=@newArrayWithSize(N);if(@isReadableByteStreamController(A)){{const S=H[0];if(!(@ArrayBuffer.@isView(S)||S instanceof @ArrayBuffer))@putByValDirect(D,0,new @Uint8Array(S.buffer,S.byteOffset,S.byteLength));else @putByValDirect(D,0,S)}for(var E=1;E<N;E++){const S=H[E];if(!(@ArrayBuffer.@isView(S)||S instanceof @ArrayBuffer))@putByValDirect(D,E,new @Uint8Array(S.buffer,S.byteOffset,S.byteLength));else @putByValDirect(D,E,S)}}else{@putByValDirect(D,0,H[0].value);for(var E=1;E<N;E++)@putByValDirect(D,E,H[E].value)}if(@resetQueue(@getByIdDirectPrivate(A,\"queue\")),@getByIdDirectPrivate(A,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(A,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(A))@readableStreamDefaultControllerCallPullIfNeeded(A);else if(@isReadableByteStreamController(A))@readableByteStreamControllerCallPullIfNeeded(A);return{value:D,size:_,done:!1}}var I=(S)=>{if(S.done)return{value:[],size:0,done:!0};var T=@getByIdDirectPrivate(K,\"readableStreamController\"),U=@getByIdDirectPrivate(T,\"queue\"),W=[S.value].concat(U.content.toArray(!1)),j=W.length;if(@isReadableByteStreamController(T))for(var F=0;F<j;F++){const d=W[F];if(!(@ArrayBuffer.@isView(d)||d instanceof @ArrayBuffer)){const{buffer:J,byteOffset:B,byteLength:G}=d;@putByValDirect(W,F,new @Uint8Array(J,B,G))}}else for(var F=1;F<j;F++)@putByValDirect(W,F,W[F].value);var w=U.size;if(@resetQueue(U),@getByIdDirectPrivate(T,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(T,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(T))@readableStreamDefaultControllerCallPullIfNeeded(T);else if(@isReadableByteStreamController(T))@readableByteStreamControllerCallPullIfNeeded(T);return{value:W,size:w,done:!1}},Q=A.@pull(A);if(Q&&@isPromise(Q))return Q.@then(I);return I(Q)})\n";
...
const char* const s_importMetaObjectLoadCJS2ESMCode = "(function (x){\"use strict\";var z=@Loader,B=@createFIFO(),F=x;while(F){var w=z.registry.@get(F);if((w\?.state\?\?0)<=@ModuleFetch)@fulfillModuleSync(F),w=z.registry.@get(F);var Q=@getPromiseInternalField(w.fetch,@promiseFieldReactionsOrResult),D=z.parseModule(F,Q),T=w.module;if(D&&@isPromise(D)){var U=@getPromiseInternalField(D,@promiseFieldReactionsOrResult),V=@getPromiseInternalField(D,@promiseFieldFlags),I=V&@promiseStateMask;if(I===@promiseStatePending||U&&@isPromise(U))@throwTypeError(`require() async module \"${F}\" is unsupported. use \"await import()\" instead.`);else if(I===@promiseStateRejected){if(!U\?.message)@throwTypeError(`${U+\"\"\?U:\"An error occurred\"} occurred while parsing module \\\"${F}\\\"`);throw U}w.module=T=U}else if(D&&!T)w.module=T=D;@setStateToMax(w,@ModuleLink);var _=T.dependenciesMap,G=z.requestedModules(T),L=@newArrayWithSize(G.length);for(var H=0,W=G.length;H<W;++H){var J=G[H],X=J[0]===\"/\"\?J:z.resolve(J,F),Y=z.ensureRegistered(X);if(Y.state<@ModuleLink)B.push(X);@putByValDirect(L,H,Y),_.@set(J,Y)}w.dependencies=L,w.instantiate=@Promise.resolve(w),w.satisfy=@Promise.resolve(w),F=B.shift();while(F&&(z.registry.@get(F)\?.state\?\?@ModuleFetch)>=@ModuleLink)F=B.shift()}var Z=z.linkAndEvaluateModule(x,@undefined);if(Z&&@isPromise(Z))@throwTypeError(`require() async module \\\"${x}\\\" is unsupported. use \"await import()\" instead.`);return z.registry.@get(x)})\n";
...
const char* const s_importMetaObjectCreateRequireCacheCode = "(function (){\"use strict\";var t=new Map,_={};return new Proxy(_,{get(f,h){const u=@requireMap.@get(h);if(u)return u;const L=@Loader.registry.@get(h);if(L\?.evaluated){const b=@Loader.getModuleNamespaceObject(L.module),c=b[@commonJSSymbol]===0||b.default\?.[@commonJSSymbol]\?b.default:b,g=@createCommonJSModule(h,c,!0);return @requireMap.@set(h,g),g}return _[h]},set(f,h,u){return @requireMap.@set(h,u),!0},has(f,h){return @requireMap.@has(h)||@Loader.registry.@has(h)},deleteProperty(f,h){return t.@delete(h),@requireMap.@delete(h),@Loader.registry.@delete(h),!0},ownKeys(f){var h=[...@requireMap.@keys()];const u=[...@Loader.registry.@keys()];for(let L of u)if(!h.includes(L))@arrayPush(h,L);return h},getPrototypeOf(f){return null},getOwnPropertyDescriptor(f,h){if(@requireMap.@has(h)||@Loader.registry.@has(h))return{configurable:!0,enumerable:!0}}})})\n";
...
const char* const s_readableStreamPipeThroughCode = "(function (y,S){\"use strict\";const c=y,d=c[\"readable\"];if(!@isReadableStream(d))@throwTypeError(\"readable should be ReadableStream\");const _=c[\"writable\"],h=@getInternalWritableStream(_);if(!@isWritableStream(h))@throwTypeError(\"writable should be WritableStream\");let u=!1,j=!1,k=!1,O;if(!@isUndefinedOrNull(S)){if(!@isObject(S))@throwTypeError(\"options must be an object\");if(j=!!S[\"preventAbort\"],k=!!S[\"preventCancel\"],u=!!S[\"preventClose\"],O=S[\"signal\"],O!==@undefined&&!@isAbortSignal(O))@throwTypeError(\"options.signal must be AbortSignal\")}if(!@isReadableStream(this))throw @makeThisTypeError(\"ReadableStream\",\"pipeThrough\");if(@isReadableStreamLocked(this))@throwTypeError(\"ReadableStream is locked\");if(@isWritableStreamLocked(h))@throwTypeError(\"WritableStream is locked\");return @readableStreamPipeToWritableStream(this,h,u,j,k,O),d})\n";
...
const char* const s_eventSourceGetEventSourceCode = "(function (){\"use strict\";class A extends EventTarget{#$;#j;#w;#A;#B;#F=!1;#G=null;#J=\"\";#K=\"\";#L=\"\";#M=!0;#O=0;#Q=0;#U=0;#V=null;static#W(W){W.#H()}static#X(W,V){const B=W.data,F=B.#L\?`Last-Event-ID: ${B.#L}\\r\\n`:\"\",G=`GET ${V.pathname}${V.search} HTTP/1.1\\r\\nHost: bun\\r\\nContent-type: text/event-stream\\r\\nContent-length: 0\\r\\n${F}\\r\\n`,X=W.write(G);if(X!==G.length)B.#K=G.substring(X)}static#Y(W,V,B){for(;;){if(B>=V.length)return;let F=-1,G=V.indexOf(\"\\r\\n\",B);const X=G+2;if(G>0)if(W.#O===0){const z=parseInt(V.substring(B,G),16);if(z===0){W.#j=2,W.#G\?.end();return}F=X+z}else F=V.length;else{if(W.#J.length===0){W.#J+=V.substring(B);return}F=V.length}let Y=V.substring(X,F);B=F+2;let J=0,Z=Y.indexOf(\"\\n\\n\");if(Z==-1){W.#J+=V.substring(X);return}if(W.#J.length)W.#J+=Y,Y=W.#J,W.#J=\"\";let K=!0;while(K){const z=Y.substring(J,Z);let L,H=\"\",w,j=0,M=-1;for(;;){let Q=z.indexOf(\"\\n\",j);if(Q===-1){if(j>=z.length)break;Q=z.length}const U=z.substring(j,Q);if(U.startsWith(\"data:\"))if(H.length)H+=`\\n${U.substring(5).trim()}`;else H=U.substring(5).trim();else if(U.startsWith(\"event:\"))L=U.substring(6).trim();else if(U.startsWith(\"id:\"))w=U.substring(3).trim();else if(U.startsWith(\"retry:\")){if(M=parseInt(U.substring(6).trim(),10),@isNaN(M))M=-1}j=Q+1}if(W.#L=w||\"\",M>=0)W.#U=M;if(H||w||L)W.dispatchEvent(new MessageEvent(L||\"message\",{data:H||\"\",origin:W.#$.origin,source:W,lastEventId:w}));if(Y.length===Z+2){K=!1;break}const O=Y.indexOf(\"\\n\\n\",Z+1);if(O===-1)break;J=Z,Z=O}}}static#Z={open(W){const V=W.data;if(V.#G=W,!V.#F)A.#X(W,V.#$)},handshake(W,V,B){const F=W.data;if(V)A.#X(W,F.#$);else F.#j=2,F.dispatchEvent(new ErrorEvent(\"error\",{error:B})),W.end()},data(W,V){const B=W.data;switch(B.#j){case 0:{let F=V.toString();const G=F.indexOf(\"\\r\\n\\r\\n\");if(G===-1){B.#J+=F;return}if(B.#J.length)B.#J+=F,F=B.#J,B.#J=\"\";const X=F.substring(0,G),Y=X.indexOf(\"\\r\\n\");if(Y===-1){B.#j=2,B.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Invalid HTTP request\")})),W.end();return}const J=X.substring(0,Y);if(J!==\"HTTP/1.1 200 OK\"){B.#j=2,B.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(J)})),W.end();return}let Z=Y+1,K=!1,z=-1;for(;;){let H=X.indexOf(\"\\r\\n\",Z);if(H===-1){if(Z>=X.length){if(!K)B.#j=2,B.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has no MIME type and \"text/event-stream\" is required. Aborting the connection.`)})),W.end();return}H=X.length}const w=X.substring(Z+1,H),j=w.indexOf(\":\"),M=w.substring(0,j),O=M.localeCompare(\"content-type\",@undefined,{sensitivity:\"accent\"})===0;if(Z=H+1,O)if(w.endsWith(\" text/event-stream\"))K=!0;else{B.#j=2,B.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has a MIME type that is not \"text/event-stream\". Aborting the connection.`)})),W.end();return}else if(M.localeCompare(\"content-length\",@undefined,{sensitivity:\"accent\"})===0){if(z=parseInt(w.substring(j+1).trim(),10),@isNaN(z)||z<=0){B.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Content-Length is invalid. Aborting the connection.`)})),W.end();return}if(K)break}else if(M.localeCompare(\"transfer-encoding\",@undefined,{sensitivity:\"accent\"})===0){if(w.substring(j+1).trim()!==\"chunked\"){B.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Transfer-Encoding is invalid. Aborting the connection.`)})),W.end();return}if(z=0,K)break}}B.#O=z,B.#j=1,B.dispatchEvent(new Event(\"open\"));const L=F.substring(G+4);if(A.#Y(B,L,0),B.#O>0){if(B.#Q+=L.length,B.#Q>=B.#O)B.#j=2,W.end()}return}case 1:if(A.#Y(B,V.toString(),2),B.#O>0){if(B.#Q+=V.byteLength,B.#Q>=B.#O)B.#j=2,W.end()}return;default:break}},drain(W){const V=W.data;if(V.#j===0){const B=V.#J;if(B.length){const F=W.write(B);if(F!==B.length)W.data.#K=B.substring(F);else W.data.#K=\"\"}}},close:A.#z,end(W){A.#z(W).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Connection closed by server\")}))},timeout(W){A.#z(W).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Timeout\")}))},binaryType:\"buffer\"};static#z(W){const V=W.data;if(V.#G=null,V.#Q=0,V.#j=2,V.#M){if(V.#V)clearTimeout(V.#V);V.#V=setTimeout(A.#W,V.#U,V)}return V}constructor(W,V=@undefined){super();const B=new URL(W);this.#F=B.protocol===\"https:\",this.#$=B,this.#j=2,process.nextTick(A.#W,this)}ref(){this.#V\?.ref(),this.#G\?.ref()}unref(){this.#V\?.unref(),this.#G\?.unref()}#H(){if(this.#j!==2)return;const W=this.#$,V=this.#F;this.#j=0,@Bun.connect({data:this,socket:A.#Z,hostname:W.hostname,port:parseInt(W.port||(V\?\"443\":\"80\"),10),tls:V\?{requestCert:!0,rejectUnauthorized:!1}:!1}).catch((B)=>{if(super.dispatchEvent(new ErrorEvent(\"error\",{error:B})),this.#M){if(this.#V)this.#V.unref\?.();this.#V=setTimeout(A.#W,1000,this)}})}get url(){return this.#$.href}get readyState(){return this.#j}close(){this.#M=!1,this.#j=2,this.#G\?.unref(),this.#G\?.end()}get onopen(){return this.#B}get onerror(){return this.#w}get onmessage(){return this.#A}set onopen(W){if(this.#B)super.removeEventListener(\"close\",this.#B);super.addEventListener(\"open\",W),this.#B=W}set onerror(W){if(this.#w)super.removeEventListener(\"error\",this.#w);super.addEventListener(\"error\",W),this.#w=W}set onmessage(W){if(this.#A)super.removeEventListener(\"message\",this.#A);super.addEventListener(\"message\",W),this.#A=W}}return Object.defineProperty(A.prototype,\"CONNECTING\",{enumerable:!0,value:0}),Object.defineProperty(A.prototype,\"OPEN\",{enumerable:!0,value:1}),Object.defineProperty(A.prototype,\"CLOSED\",{enumerable:!0,value:2}),A[Symbol.for(\"CommonJS\")]=0,A})\n";
